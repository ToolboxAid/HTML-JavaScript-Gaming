...
- Moved tracked `docs_build/` content to `dev/docs_build/`.
...
- Updated active Project Instructions, PR workflow references, package scripts, validation scripts, and tests to use moved `dev/docs_build/...` and `dev/archive/...` paths.
- Updated dev-runtime Admin Notes paths to the existing moved archive source at `dev/archive/docs_build/dev/admin-notes/`.
- Updated dev-only toolbox imports and guard baselines so moved validation scripts resolve from `dev/docs_build/dev/toolbox/`.
...
- Moved destination check: PASS, `dev/docs_build/`, `dev/archive/`, and `dev/project-instructions/` exist.
...
- `node dev/docs_build/dev/toolbox/checkIntentionalAliasLedgerGuard.mjs` still reports pre-existing alias ledger debt unrelated to this path move.
- `node dev/docs_build/dev/toolbox/checkInternalBarrelGuard.mjs` still reports pre-existing barrel policy debt unrelated to this path move.
