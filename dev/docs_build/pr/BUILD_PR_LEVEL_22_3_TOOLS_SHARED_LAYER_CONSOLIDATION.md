...
- keep: `src/shared/toolbox/`
- eliminate: `docs_build/dev/toolbox/shared/`

## Scope
Included:
- inventory files in both shared locations
- classify duplicates / overlaps / unique files
- move or merge valid shared files from `docs_build/dev/toolbox/shared/` into `src/shared/toolbox/`
...
Inventory:
- all files under `src/shared/toolbox/`
- all files under `docs_build/dev/toolbox/shared/`

For each file classify as one of:
- keep in `toolbox/shared`
- move from `docs_build/dev/toolbox/shared` to `toolbox/shared`
- merge into existing `toolbox/shared` file
- remove as obsolete duplicate

### 2. Consolidation rules
- `src/shared/toolbox/` becomes the only shared location
- no file may remain under `docs_build/dev/toolbox/shared/` after consolidation unless explicitly justified in validation
...
- no unresolved references to `docs_build/dev/toolbox/shared/`
- no start_of_day changes
- unrelated working-tree changes preserved

## Acceptance
- one authoritative shared tool layer exists
- `src/shared/toolbox/` is authoritative
- `docs_build/dev/toolbox/shared/` is eliminated or reduced to zero in-scope files
