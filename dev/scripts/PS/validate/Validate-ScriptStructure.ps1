...
$scriptsRoot = Join-Path $repoRoot "dev\scripts\PS"
...
foreach ($misplaced in @(
    ...
    foreach ($name in $misplaced) {
        $issues.Add("Misplaced deployment script '$name' in dev/scripts/PS/codex. Move to dev/scripts/PS/deploy.")
    }
}

foreach ($name in (Get-FileNames -Directory $deployRoot | Where-Object { $_ -like "*Codex*" })) {
    $issues.Add("Misplaced codex script '$name' in dev/scripts/PS/deploy. Move to dev/scripts/PS/codex.")
}
...
foreach ($name in (Get-FileNames -Directory $scriptsRoot | Where-Object { $_ -like "*Codex*" -or $_ -like "*WebsiteRepoDeployment*" -or $_ -like "Validate-*" })) {
    $issues.Add("Misplaced specialized script '$name' in dev/scripts/PS. Move to dev/scripts/PS/codex, dev/scripts/PS/deploy, or dev/scripts/PS/validate.")
}

foreach ($name in (Get-FileNames -Directory $validateRoot | Where-Object { $_ -notlike "Validate-*" })) {
    $issues.Add("Naming mismatch in dev/scripts/PS/validate: '$name'. Validation scripts should start with 'Validate-'.")
}
