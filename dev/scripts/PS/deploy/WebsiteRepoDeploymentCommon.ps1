...
function Get-DeployExpectedScriptsRoot {
    return [System.IO.Path]::GetFullPath((Join-Path (Get-DeployRepoRoot) "dev\scripts\PS\deploy"))
}
...
function Assert-DeployScriptLocation {
    ...
    if (-not [string]::Equals(
        ...
        throw "Deployment script placement check failed. Expected scripts under '$expectedRoot' but script was loaded from '$resolvedScriptRoot'. Run dev/scripts/PS/validate/Validate-ScriptStructure.ps1 for details."
    }
}
