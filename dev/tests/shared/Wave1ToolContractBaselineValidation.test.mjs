...
David Quesenberry
06/02/2026
Wave1ToolContractBaselineValidation.test.mjs
*/
...
const scenariosPath = fileURLToPath(
  new URL("../fixtures/project-workspaces/wave-1-tool-migration-scenarios.json", import.meta.url)
);

export const WAVE_1_TOOL_IDS = Object.freeze([
  "state-inspector",
  "replay-visualizer",
...
export function run() {
  ...
  for (const tool of scenarios.tools) {
    const toolContract = getWave1ToolContract(tool.toolId);
...
export function getWave1ToolContract(toolId) {
  const toolContract = TOOL_CONTRACT_LIST.find((contract) => contract.toolId === toolId);
  assert.ok(toolContract, `Wave 1 tool contract exists: ${toolId}`);
  return toolContract;
}
