...
export async function run() {
  ...
  assert.equal(ready.stages.emit.coordinatorPath, "archive/v1-v2/games/asteroids/assets/tools.manifest.json");
  assert.equal(ready.stages.emit.gameAssetManifestPath, "archive/v1-v2/games/asteroids/assets/tools.manifest.json");
  assert.equal(
  ...
  assert.equal(ready.coordinator.domains.sprites[0].runtimePath.startsWith("archive/v1-v2/games/asteroids/assets/sprites/"), true);
  assert.equal(ready.coordinator.domains.sprites[0].toolDataPath.startsWith("archive/v1-v2/games/asteroids/assets/sprites/data/"), true);
  assert.equal(ready.gameAssetManifest.status, "ready");
  assert.equal(ready.gameAssetManifest.filePath, "archive/v1-v2/games/asteroids/assets/tools.manifest.json");
  assert.equal(ready.gameAssetManifest.manifest.domains.sprites[0].assetId, "sprite.ship");
  assert.equal(
    ready.gameAssetManifest.manifest.domains.sprites[0].runtimePath.startsWith("archive/v1-v2/games/asteroids/assets/sprites/"),
    true
  );
  assert.equal(
    ready.gameAssetManifest.manifest.domains.sprites[0].toolDataPath.startsWith("archive/v1-v2/games/asteroids/assets/sprites/data/"),
    true
  );
