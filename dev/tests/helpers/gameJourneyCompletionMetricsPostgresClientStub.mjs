...
function filterFromQuery(query = "") {
  ...
  for (const [key, value] of params.entries()) {
    ...
    if (!value.startsWith("eq.")) {
      throw new Error(`Unsupported Game Journey metrics Postgres test filter for ${key}.`);
    }
    return {
      key,
...
export function createGameJourneyCompletionMetricsPostgresClientStub() {
  ...
  return {
    ...
    async requestTable(tableName, { body = null, method = "GET", query = "select=*" } = {}) {
      ...
      throw new Error(`Unsupported Game Journey metrics Postgres test method: ${normalizedMethod}.`);
    },
  };
}
