...
const METADATA_URL = '/old_samples/metadata/samples.index.metadata.json';
const BACK_TO_SAMPLES_HREF = '/old_samples/index.html';
...
export function parseSampleFromPathname(pathname) {
  ...
  const direct = text.match(/\/old_samples\/phase-?(\d{2})\/(\d{4})(?:\/index\.html)?\/?$/);
  if (direct) {
...
export function canonicalSampleHref(sample) {
  return '/old_samples/phase-' + sample.phase + '/' + sample.id + '/index.html';
}
