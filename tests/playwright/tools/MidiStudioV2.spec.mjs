...
async function visibleMidiStudioControlOwnership(page, activeTabId) {
  return page.evaluate((tabId) => {
    ...
    const classify = (element) => {
      ...
      if (element.dataset.laneInstrumentSummary) {
        return { canonical: "derived from music.songs[].studioArrangement.previewLaneSettings", kind: "readonly", owner: "Octave Timeline", wired: "wired" };
      }
    ...
    const ownershipSlot = (element, ownership) => {
      const laneKeys = [
        "instrumentDisplayNameLane",
        "laneInstrumentTypeSelect",
        "laneInstrumentSelect",
        "previewVolumeLane",
        "previewPanLane",
        "previewMuteLane",
        "previewSoloLane",
        "instrumentOctaveLowLane",
      ...
      if (element.dataset.songDetailField) {
        return `${ownership.canonical}::${element.dataset.songDetailField}`;
      }
      if (element.dataset.renderedTargetFormat) {
        return `${ownership.canonical}::${element.dataset.renderedTargetFormat}`;
      }
      if (element.dataset.songSheetSectionName) {
        return `${ownership.canonical}::${element.dataset.songSheetSectionName}`;
      }
      return `${ownership.canonical}::${element.id || controlText(element)}`;
    };
    return Array.from(document.querySelectorAll(selector))
      ...
      .map((element) => {
        ...
        return {
          ...
          slot: ownershipSlot(element, ownership),
...
async function expectMidiStudioReleaseCandidateControlAudit(page, tabIds) {
  const controls = await expectMidiStudioVisibleControlAudit(page, tabIds);
  const intentionalMultiEditCanonicals = [
    "music.songs[].director.usage",
    "music.songs[].studioArrangement.previewLaneSettings.muted",
    "music.songs[].studioArrangement.previewLaneSettings.soloed"
  ];
  const duplicateEditableSlots = Array.from(controls
    .filter((control) => control.editable && control.kind === "canonical")
    .filter((control) => !intentionalMultiEditCanonicals.includes(control.canonical))
    .reduce((groups, control) => {
      const entry = groups.get(control.slot) || [];
      entry.push(control);
      groups.set(control.slot, entry);
      return groups;
    }, new Map()))
    .filter(([, entries]) => entries.length > 1)
    .map(([slot, entries]) => ({
      slot,
      controls: entries.map((entry) => `${entry.activeTabId}:${entry.control}`)
    }));
  expect(duplicateEditableSlots).toEqual([]);

  const falseUnwired = controls
    .filter((control) => control.wired === "wired" && control.unwiredClass)
    .map((control) => `${control.activeTabId}:${control.control}`);
  expect(falseUnwired).toEqual([]);

  const unmarkedFutureControls = controls
    .filter((control) => control.wired === "unwired")
    .filter((control) => !control.unwired || !control.unwiredClass || !control.title)
    .map((control) => `${control.activeTabId}:${control.control}`);
  expect(unmarkedFutureControls).toEqual([]);
  return controls;
}
...
test.describe("MIDI Studio V2", () => {
  ...
  test("octave timeline remains editable and playable after Song Setup default tab", async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator('[data-midi-studio-tab="export"]')).toHaveCount(1);
  ...
  test("imports UAT manifest and plays the upbeat multi-instrument studio arrangement", async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator("#instrumentGridOutput [data-lane-instrument-select], #instrumentGridOutput [data-lane-instrument-type-select]")).toHaveCount(0);
      await expect(page.locator("#instrumentGridOutput [data-lane-volume-slider], #instrumentGridOutput [data-lane-pan-slider]")).toHaveCount(0);
      await expect(laneHeader(page, "lead").locator("[data-lane-label='lead']")).toHaveText("Retro Pulse Lead");
      await expect(laneHeader(page, "bass").locator("[data-lane-label='bass']")).toHaveText("Synth Bass");
      await expect(laneHeader(page, "drums").locator("[data-lane-label='drums']")).toHaveText("Basic Drums");
      await expect(laneHeader(page, "lead").locator("[data-lane-instrument-summary='lead']")).toContainText("Retro Pulse Lead");
      await selectMidiStudioTab(page, "instruments");
      await selectInstrumentRow(page, "lead");
      await expect(instrumentSelect(page, "lead")).toHaveValue("retro-pulse-lead");
      await expect(instrumentSelect(page, "bass")).toHaveCount(0);
      await expect(instrumentTypeSelect(page, "lead")).toHaveValue("Synth");
      await instrumentTypeSelect(page, "lead").selectOption("Keyboard");
      await expect(instrumentSelect(page, "lead")).toHaveValue("preview-electric-piano");
      await expect(laneHeader(page, "lead").locator("[data-lane-label='lead']")).toHaveText("Preview Electric Piano");
      expect(await instrumentSelect(page, "lead").locator("option").evaluateAll((options) => options.map((option) => option.textContent))).toEqual([
        "Choose instrument",
      ...
      await instrumentSelect(page, "lead").selectOption("warm-pad");
      await selectMidiStudioTab(page, "studio");
      ...
      await expect(laneHeader(page, "lead").locator('[data-lane-control-toggle="volume"]')).toHaveCount(0);
      await expect(laneHeader(page, "lead").locator('[data-lane-control-toggle="pan"]')).toHaveCount(0);
      ...
      await page.locator("#timelineAddInstrumentRowButton").click();
      await expect(laneHeader(page, "instrument-1")).toBeVisible();
      await expect(laneInstrumentTypeSelect(page, "instrument-1")).toHaveCount(0);
  ...
  test("validates PR081-084 song builder generation and timeline section visibility", async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator("[data-song-sheet-section-metrics='Bridge']")).toContainText("Empty - add chords");
  ...
  test("validates PR109-116 overnight MIDI Studio completion polish", async ({ page }) => {
    await page.setViewportSize({ width: 1600, height: 900 });
    const server = await openMidiStudioForImport(page);
    try {
      await page.locator("#toolImportManifestInput").setInputFiles(uatManifestPath);
      ...
      await page.locator("#gameUsageMenuInput").check();
      await page.locator("#gameUsageLoopInput").check();
      await page.locator("#gameUsageCustomInput").fill("Night");
      await expect(page.locator("#gameUsageSummary")).toContainText("Menu, Loop, Night");

      await page.locator("#songSheetTempoInput").fill("960");
      await fillSongSheetSectionBuilder(page, "Intro:\nVerse:\nChorus:\nBridge:\nOutro:");
      await expect(page.locator("#songSheetAvailableSectionsList option")).toHaveText(["Populate a section to make it available."]);
      await expect(page.locator("#songSheetAvailableSectionsList option")).toHaveAttribute("data-song-sheet-available-sections-empty", "true");
      await expect(page.locator("#songSheetAddSectionToSequenceButton")).toBeDisabled();
      await expect(page.locator("[data-song-sheet-section-metrics='Intro']")).toContainText("Empty - add chords");
      await expect(page.locator("[data-song-sheet-custom-section-metrics]")).toContainText("Empty - add CustomName");
      ...
      await expect(page.locator("#songSheetAvailableSectionsList option")).toHaveText([
        ...
        "Chorus - 1 bar / 1 chord",
        "Outro - 1 bar / 1 chord",
        "CustomCue - 1 bar / 1 chord"
      ]);
      await expect(page.locator("#songSheetAvailableSectionsList option", { hasText: "Bridge" })).toHaveCount(0);
      await expect(page.locator("[data-song-sheet-section-metrics='Intro']")).toContainText("0.25 seconds");

      await clearSongSheetSequence(page);
      await addSongSheetSequenceLabels(page, ["Intro", "Verse", "Chorus", "CustomCue"]);
      await page.locator("#songSheetSequenceList").selectOption({ index: 1 });
      await page.locator("#songSheetDuplicateSequenceButton").click();
      await expect(page.locator("#songSheetSequenceList option")).toHaveText(["Intro", "Verse", "Verse", "Chorus", "CustomCue"]);
      await page.locator("#songSheetSequenceMoveDownButton").click();
      await page.locator("#songSheetSequenceRemoveButton").click();
      await addSongSheetSequenceLabels(page, ["Intro", "Verse", "Chorus", "CustomCue", "Outro"]);
      await expect(page.locator("#songSheetSequenceSummary")).toContainText("5 sections");
      await expect(page.locator("#songSheetSequenceSummary")).toContainText("5 bars");
      await expect(page.locator("#songSheetApplyChordsPadInput")).toBeChecked();
      await expect(page.locator("#songSheetApplyBassInput")).toBeChecked();
      await expect(page.locator("#songSheetApplyLeadInput")).not.toBeChecked();

      await page.locator("#parseSongSheetButton").click();
      await expect(page.locator("#songSheetSummary [data-song-sheet-summary-field='sequence'] dd")).toHaveText("Intro, Verse, Chorus, CustomCue, Outro");
      await expect(page.locator("#songSheetSummary [data-song-sheet-summary-field='generated-bars'] dd")).toHaveText("5");
      await expect(page.locator("#songSheetWarningsDetails [data-song-sheet-warning-field='song-sheet-warnings']")).toContainText("none");
      await expect(page.locator("#instrumentGridSectionsInput")).toHaveValue("Intro:1, Verse:1, Chorus:1, CustomCue:1, Outro:1");
      await expect(page.locator("#inspectorOutput")).toContainText('"classification": "Loop"');
      await expect(page.locator("#inspectorOutput")).toContainText('"sequence": "Intro, Verse, Chorus, CustomCue, Outro"');
      await expect(page.locator("#audioDiagnostics")).toContainText("Playable note count");

      await selectMidiStudioTab(page, "studio");
      await waitForCanvasRender(page);
      await expect(octaveTimelineCanvas(page)).toHaveAttribute("data-section-header-labels", "Intro|Verse|Chorus|CustomCue|Outro");
      await page.locator("#instrumentGridOutput").evaluate((output) => {
        output.scrollTop = 220;
        output.scrollLeft = 160;
        output.dispatchEvent(new Event("scroll"));
      });
      await expect(octaveTimelineCanvas(page)).toHaveAttribute("data-frozen-header", "true");
      await expect(octaveTimelineCanvas(page)).toHaveAttribute("data-playhead-bar", /\d+/);
      await expect(octaveTimelineCanvas(page)).toHaveAttribute("data-playhead-beat", /\d+/);
      await page.locator("#instrumentGridOutput").evaluate((output) => {
        output.scrollTop = 0;
        output.scrollLeft = 0;
        output.dispatchEvent(new Event("scroll"));
      });
      await clickCanvasSectionHeader(page, "CustomCue", 0);
      await expect(page.locator("#songSheetSequenceList")).toHaveAttribute("data-song-sheet-selected-section", "CustomCue");
      ...
      await expect(page.locator("#playButton")).toBeEnabled();
      await page.locator("#instrumentGridLoopStartSelect").selectOption("Intro");
      await page.locator("#instrumentGridLoopEndSelect").selectOption("Chorus");
      await page.locator("#playLoopButton").click();
      await expect(page.locator("#playbackState")).toContainText("(looping)");
      await expect(page.locator("#instrumentGridOutput")).toHaveAttribute("data-preview-playback-mode", "loop");
      await page.locator("#stopTimingPreviewButton").click();
      ...
      await selectMidiStudioTab(page, "instruments");
      await expect(instrumentRow(page, "lead")).toHaveClass(/is-selected/);
      await setInputValue(page, "#previewOctaveLowLeadInput", "3");
      await setInputValue(page, "#previewOctaveHighLeadInput", "5");
      await setInputValue(page, "#previewVolumeLeadInput", "0.75");
      await expect(page.locator("#instrumentAuditionKeyboard")).toContainText("C3");
      await expect(page.locator("#instrumentAuditionKeyboard")).toContainText("B5");
      await expect(page.locator("#selectedInstrumentEditor [data-midi-studio-field-state='unwired']")).toHaveCount(8);
      expect(await page.evaluate(() => {
        const settings = window.__midiStudioV2App.selectedSong().studioArrangement.previewLaneSettings;
        return {
          octaveRange: settings.octaveRanges.lead,
          selected: window.__midiStudioV2App.instrumentGrid.selectedInstrumentId,
          volume: settings.volumes.lead
        };
      })).toEqual({ octaveRange: { high: 5, low: 3 }, selected: "lead", volume: 0.75 });
      ...
      await instrumentRow(page, "lead-1").locator("[data-delete-instrument-row='lead-1']").click();
      await expect(page.locator("[data-delete-confirmation-lane='lead-1']")).toBeVisible();
      await page.locator("[data-cancel-delete-instrument-row='lead-1']").click();
      await expect(instrumentRow(page, "lead-1")).toBeVisible();

      await selectMidiStudioTab(page, "export");
      await expect(page.locator("#exportRenderSource [data-export-field='classification'] dd")).toHaveText("Loop");
      await expect(page.locator("#exportRenderSource [data-export-field='generated-id'] dd")).toHaveText("camptownRacesUatReel-Loop");
      await expect(page.locator("#exportRenderSource [data-export-field='game-usage-assignment'] dd")).toContainText("Menu, Loop, Night");
      await expect(page.locator("#exportManifestReadinessDetails [data-export-field='game-usage-assignment'] dd")).toContainText("songs assigned");
      await expect(page.locator("#exportManifestReadinessDetails [data-export-field='export-readiness'] dd")).toContainText("Game Usage");
      await expect.poll(async () => Number(await page.locator("#exportRenderSource [data-export-field='note-count'] dd").textContent())).toBeGreaterThan(0);
      await expect(page.locator("#renderedExportSaveButton")).toHaveText("Save WAV");
      await expect(page.locator("#futureSoundFontSelect")).toHaveAttribute("data-midi-studio-unwired", "not-implemented");
      await page.locator("#renderedExportSaveButton").click();
      await expect(page.locator("#exportStatusDetails [data-export-field='renderer'] dd")).toHaveText("Not implemented");
      await expect(page.locator("#exportStatusDetails [data-export-field='status'] dd")).toContainText(/WARN|FAIL/);
      await expect(page.locator("#futureGameUsageRuntimeSyncButton")).toHaveAttribute("data-midi-studio-unwired", "not-implemented");
      await expect(page.locator("#songSheetDragDropSequenceButton")).toHaveAttribute("data-midi-studio-unwired", "not-implemented");

      const controls = [];
      for (const tabId of ["song-setup", "studio", "instruments", "auto-create-parts", "diagnostics", "export"]) {
        await selectMidiStudioTab(page, tabId);
        if (tabId === "studio") {
          await waitForCanvasRender(page);
        }
  ...
  test("validates PR117-124 library generation import diagnostics and manifest hardening", async ({ page }) => {
    await page.setViewportSize({ width: 1600, height: 900 });
    const server = await openMidiStudioForImport(page);
    try {
      await page.locator("#toolImportManifestInput").setInputFiles(uatManifestPath);
      ...
      await expect(page.locator("#songLibrarySummary")).toHaveAttribute("data-song-library-count", "1");
      await page.locator("#saveSongButton").click();
      await expect(page.locator("#songLibrarySummary")).toContainText("Updated song asset");
      ...
      await expect(page.locator("#songLibrarySummary")).toContainText("Loaded song asset into canonical songs");
      expect(await page.evaluate(() => {
        const ids = window.__midiStudioV2App.payload.songs.map((song) => song.id);
        return {
          ids,
      ...
      await fillSongSheetSectionBuilder(page, "Intro:\nVerse:\nChorus:\nBridge:\nOutro:");
      await expect(page.locator("#songSheetSaveSectionButton")).toBeDisabled();
      await expect(page.locator("#songSheetSectionLibrarySummary")).toHaveAttribute("data-song-sheet-section-library-count", "0");
      ...
      await expect(page.locator("#statusLog")).toHaveValue(/OK Saved section asset: Intro\./);
      await page.locator("#songSheetLoadSectionButton").click();
      await expect(page.locator("#songSheetSectionLibrarySummary")).toContainText("Loaded section asset: Intro");
      await page.locator("#songSheetDuplicateSectionButton").click();
      await expect(page.locator("#songSheetCustomSectionsInput")).toHaveValue(/IntroCopy1: C G/);
      await expect(page.locator("#songSheetSectionLibrarySummary")).toHaveAttribute("data-song-sheet-section-library-count", "2");
      ...
      await page.locator("#parseSongSheetButton").click();
      await expect(page.locator("#songSheetSummary [data-song-sheet-summary-field='sequence'] dd")).toHaveText("Intro, Verse, Chorus, CustomCue, Outro");
      await page.locator("#songSheetApplyLeadInput").check();

      await selectMidiStudioTab(page, "studio");
      await waitForCanvasRender(page);
      await timelineQuickInstrumentRow(page, "lead").locator("[data-quick-instrument-label='lead']").click();
      const manualTarget = await emptyCanvasRun(page, { lane: "lead", length: 1 });
      expect(manualTarget).toBeTruthy();
      await clickCanvasCell(page, manualTarget.rowToken, manualTarget.stepIndex);
      await selectMidiStudioTab(page, "song-setup");
      await page.locator("#regenerateArrangementButton").click();
      ...
      await expect(page.locator("#statusLog")).toHaveValue(/WARN Regeneration may overwrite \d+ manual note/);
      await page.locator("#regenerateArrangementButton").click();
      await expect(page.locator("#regenerateArrangementButton")).toHaveText("Regenerate Arrangement");
      ...
      await expect(page.locator('.accordion-v2__header[aria-controls="instrumentGridSummaryContent"]')).toContainText("Timeline Diagnostics");
      await expect(page.locator('.accordion-v2__header[aria-controls="audioDiagnosticsContent"]')).toContainText("Audio Diagnostics");
      await expect(page.locator('.accordion-v2__header[aria-controls="diagnosticsManifestReadinessContent"]')).toContainText("Manifest Readiness");
      await expect(page.locator('.accordion-v2__header[aria-controls="renderedTargetDiagnosticsContent"]')).toContainText("Export Readiness");
      await expect(page.locator('.accordion-v2__header[aria-controls="statusLogContent"]')).toContainText("Status");
      await expect(page.locator("#diagnosticsManifestReadinessDetails [data-export-field='game-usage-assignment'] dd")).toContainText("Menu: none");
      await expect(page.locator("#diagnosticsManifestReadinessDetails [data-export-field='game-usage-assignment'] dd")).toContainText("Custom:");
      await expect(page.locator("#diagnosticsManifestReadinessDetails [data-export-field='game-usage-assignment'] dd")).toContainText("Missing assignments WARN");
      await expect(page.locator("#renderedTargetDiagnostics [data-export-field='status'] dd")).toContainText(/WARN|INFO/);
      const diagnosticEditableControls = await page.locator('[data-midi-studio-tab-panel="diagnostics"] input:not([type="hidden"]), [data-midi-studio-tab-panel="diagnostics"] select, [data-midi-studio-tab-panel="diagnostics"] textarea:not([readonly])').count();
      expect(diagnosticEditableControls).toBe(0);
  ...
  test("validates PR117-124 workspace handoff nav and tool-only JSON workflows", async ({ page }) => {
    const manifest = JSON.parse(await fs.readFile(uatManifestPath, "utf8"));
    ...
    try {
      ...
      await waitForCanvasRender(page);
      await timelineQuickInstrumentRow(page, "lead").locator("[data-quick-instrument-label='lead']").click();
      await clickCanvasCell(page, "C6", 3);
      expect(await page.evaluate(() => {
        const session = JSON.parse(window.sessionStorage.getItem("workspace.tools.midi-studio-v2"));
        const song = session.data.songs.find((candidate) => candidate.id === session.data.activeSongId);
        return {
          dirty: session.dirty.isDirty,
          lead: song.studioArrangement.lanes.lead
        };
      })).toEqual(expect.objectContaining({
        dirty: true
      }));
    ...
    try {
      ...
      await page.locator("#toolImportManifestInput").setInputFiles(uatManifestPath);
      await expect(page.locator("#toolExportToolStateButton")).toBeEnabled();
      await page.locator("#toolExportToolStateButton").click();
      await expect(page.locator("#inspectorOutput")).toContainText('"toolId": "midi-studio-v2"');
      await expect(page.locator("#statusLog")).toHaveValue(/OK MIDI Studio V2 toolState preview written to JSON Details\./);
    } finally {
      await workspaceV2CoverageReporter.stop(page);
      await server.close();
    }
  ...
  test("validates PR125-132 codebase completion SSoT song sheet timeline instruments playback and export", async ({ page }) => {
    await page.setViewportSize({ width: 1600, height: 900 });
    const server = await openMidiStudioForImport(page);
    try {
      await page.locator("#toolImportManifestInput").setInputFiles(uatManifestPath);
      ...
      await page.locator("#songSheetSequenceRemoveButton").click();
      await expect(page.locator("#songSheetSequenceList option")).toHaveText(["Intro", "Verse", "Chorus"]);
      for (const label of ["CustomCue", "Outro"]) {
        await page.locator("#songSheetAvailableSectionsList").selectOption(label);
        await page.locator("#songSheetAddSectionToSequenceButton").click();
      }
      await expect(page.locator("#songSheetSequenceSummary")).toContainText("5 sections");

      await page.locator("#songSheetApplyChordsPadInput").check();
      await page.locator("#songSheetApplyBassInput").check();
      await page.locator("#songSheetApplyDrumsInput").uncheck();
      await page.locator("#songSheetApplyLeadInput").check();
      await page.locator("#parseSongSheetButton").click();
      await expect(page.locator("#songSheetSummary [data-song-sheet-summary-field='sequence'] dd")).toHaveText("Intro, Verse, Chorus, CustomCue, Outro");
      await expect(page.locator("#songSheetSummary [data-song-sheet-summary-field='generation-targets'] dd")).toHaveText("Chords/Pad, Bass, Lead");
      await expect(page.locator("#instrumentGridSectionsInput")).toHaveValue("Intro:2, Verse:4, Chorus:2, CustomCue:2, Outro:1");
      await expect(page.locator("#statusLog")).toHaveValue(/OK Song Sheet updated the editable note grid for Chords\/Pad, Bass, Lead: 11 bars/);

      const canonicalAfterParse = await page.evaluate(() => {
        const app = window.__midiStudioV2App;
        const song = app.selectedSong();
        const state = app.selectedSongState();
        return {
          activeSongId: app.payload.activeSongId,
          barCount: app.currentInstrumentGridResult().barCount,
          classification: song.classification,
          generatedId: song.id,
          hasOwnSelectedSongId: Object.hasOwn(app, "selectedSongId"),
          instrumentCount: Object.keys(song.studioArrangement.lanes).length,
          key: song.studioArrangement.key,
          modelSongId: state.songId,
          name: song.name,
          sequence: song.studioArrangement.songSheet.sequence,
          style: song.studioArrangement.style,
          tempo: song.studioArrangement.tempo
      ...
      expect(canonicalAfterParse).toEqual(expect.objectContaining({
        activeSongId: "camptownRacesUatReel-Loop",
        barCount: 11,
        classification: "Loop",
        generatedId: "camptownRacesUatReel-Loop",
        hasOwnSelectedSongId: false,
        modelSongId: "camptownRacesUatReel-Loop",
        name: "Camptown Races UAT Reel",
        sequence: "Intro, Verse, Chorus, CustomCue, Outro",
        tempo: "960"
      }));
      expect(canonicalAfterParse.instrumentCount).toBeGreaterThan(4);
      ...
      await expect(page.locator("#instrumentGridOutput [data-lane-volume-slider], #instrumentGridOutput [data-lane-pan-slider]")).toHaveCount(0);
      await timelineQuickInstrumentRow(page, "lead").locator("[data-quick-instrument-label='lead']").click();
      const editTarget = await emptyCanvasRun(page, { lane: "lead", length: 2 });
      ...
      await expect(page.locator("#inspectorOutput")).toContainText('"classification": "Loop"');
      await expect(page.locator("#inspectorOutput")).toContainText('"sequence": "Intro, Verse, Chorus, CustomCue, Outro"');
      await expect(page.locator("#instrumentGridSummary")).toContainText("Bars");
      await expect(page.locator("#diagnosticsManifestReadinessDetails")).toContainText("Missing assignments WARN");
      ...
      await expect(page.locator("#exportRenderSource [data-export-field='classification'] dd")).toHaveText("Loop");
      await expect(page.locator("#exportRenderSource [data-export-field='generated-id'] dd")).toHaveText("camptownRacesUatReel-Loop");
      await expect(page.locator("#exportRenderSource [data-export-field='sequence-length'] dd")).toHaveText("5");
      await expect.poll(async () => Number(await page.locator("#exportRenderSource [data-export-field='note-count'] dd").textContent())).toBeGreaterThan(0);
      await expect(page.locator("#renderedExportSaveButton")).toHaveText("Save WAV");
      await page.locator("#renderedExportTargetTypeSelect").selectOption("mp3");
      await expect(page.locator("#renderedExportSaveButton")).toHaveText("Save MP3");
      ...
      await expect(page.locator("#renderedExportSaveButton")).toHaveText("Save OGG");
      await expect(page.locator("#futureSoundFontSelect")).toHaveAttribute("data-midi-studio-unwired", "not-implemented");
      await expect(page.locator("#futureSoundFontSelect")).toHaveClass(/midi-studio-v2__unwired-control/);
      await setInputValue(page, "#renderedTargetOggInput", "assets/music/rendered/camptown-races-uat-reel.ogg");
      await page.locator("#renderedExportSaveButton").click();
      await expect(page.locator("#statusLog")).toHaveValue(/WARN Export rendering not implemented for OGG\./);
  ...
  test("validates PR125-132 workspace and tool launch ownership split", async ({ page }) => {
    const manifest = JSON.parse(await fs.readFile(uatManifestPath, "utf8"));
    let server = await openMidiStudioFromWorkspace(page, manifest);
    try {
      await expect(page.locator("body")).toHaveAttribute("data-midi-studio-launch-mode", "workspace");
      ...
      await waitForCanvasRender(page);
      await timelineQuickInstrumentRow(page, "lead").locator("[data-quick-instrument-label='lead']").click();
      const workspaceEditTarget = await emptyCanvasRun(page, { lane: "lead", length: 1 });
      expect(workspaceEditTarget).toBeTruthy();
      await clickCanvasCell(page, workspaceEditTarget.rowToken, workspaceEditTarget.stepIndex);
      expect(await page.evaluate(() => {
        const session = JSON.parse(window.sessionStorage.getItem("workspace.tools.midi-studio-v2"));
        const song = session.data.songs.find((candidate) => candidate.id === session.data.activeSongId);
        return {
          dirty: session.dirty.isDirty,
          hasLeadEdit: song.studioArrangement.lanes.lead.length > 0
        };
      })).toEqual({ dirty: true, hasLeadEdit: true });
    } finally {
      await workspaceV2CoverageReporter.stop(page);
      await server.close();
    }
  ...
  test("validates PR133-140 finish-line tool persistence SSoT UI song builder timeline instruments export and playback", async ({ page }) => {
    await page.setViewportSize({ width: 1600, height: 900 });
    const server = await openMidiStudioForImport(page);
    try {
      expect(await page.evaluate(() => ({
      ...
      await expect(page.locator("#statusLog")).toHaveValue(/PASS Import JSON PASS: canonical payload loaded with 3 songs/);
      await selectMidiStudioTab(page, "song-setup");
      ...
      await addSongSheetSequenceLabels(page, ["Intro", "Verse", "Chorus", "CustomCue", "Outro"]);
      await page.locator("#songSheetDuplicateSequenceButton").click();
      await expect(page.locator("#songSheetSequenceList option")).toHaveText(["Intro", "Verse", "Chorus", "CustomCue", "Outro", "Outro"]);
      await page.locator("#songSheetSequenceRemoveButton").click();
      await page.locator("#songSheetApplyChordsPadInput").setChecked(true);
      await page.locator("#songSheetApplyBassInput").setChecked(true);
      await page.locator("#songSheetApplyDrumsInput").setChecked(false);
      ...
      await songDetailsHeader.click({ position: { x: 8, y: 8 } });
      await expect(songDetailsHeader).toHaveAttribute("aria-expanded", "false");
      await expect(songDetailsHeader.locator(".accordion-v2__icon")).toHaveText("+");
      await songDetailsHeader.click({ position: { x: 8, y: 8 } });
      await expect(songDetailsHeader).toHaveAttribute("aria-expanded", "true");
      await expect(songDetailsHeader.locator(".accordion-v2__icon")).toHaveText("X");
      await expect(page.locator("#songSheetDragDropSequenceButton")).toHaveAttribute("data-midi-studio-unwired", "not-implemented");
      ...
      await expect(page.locator("#futureSoundFontSelect")).toHaveAttribute("data-midi-studio-unwired", "not-implemented");
      await expect(page.locator("#renderedExportSaveButton")).toHaveAttribute("data-midi-studio-unwired", "not-implemented");
      await setInputValue(page, "#renderedTargetOggInput", "assets/music/rendered/loop.ogg");
      await page.locator("#renderedExportSaveButton").click();
      await expect(page.locator("#statusLog")).toHaveValue(/WARN Export rendering not implemented for (WAV|OGG)|FAIL Missing rendered WAV export target/);
      await expect(page.locator("#statusLog")).not.toHaveValue(/created .*\.ogg|wrote .*\.ogg|saved .*\.ogg/i);
      await page.locator("#toolExportToolStateButton").click();
      ...
      const exportedSong = exportedToolState.payload.songs.find((song) => song.id === "camptownRacesUatReel-Loop");
      expect(exportedSong).toEqual(expect.objectContaining({
        classification: "Loop",
        id: "camptownRacesUatReel-Loop"
      }));
      expect(exportedSong.studioArrangement.songSheet.sequence).toBe("Intro, Verse, Chorus, CustomCue, Outro");
      ...
      expect(await page.evaluate(() => window.sessionStorage.getItem("workspace.tools.midi-studio-v2"))).toBeNull();
      ...
      await page.locator("#toolImportManifestInput").setInputFiles({
        ...
        name: "midi-studio-v2-roundtrip.tool-state.json"
      });
      ...
      const roundTrip = await page.evaluate((target) => {
        const app = window.__midiStudioV2App;
        const song = app.selectedSong();
        const settings = song.studioArrangement.previewLaneSettings;
        return {
          classification: song.classification,
          generatedId: song.id,
          leadHasManualNote: String(song.studioArrangement.lanes.lead || "").includes(target.rowToken),
          sequence: song.studioArrangement.songSheet.sequence,
          settings: {
            pan: settings.pans.lead,
            transpose: settings.transposes.lead,
      ...
      expect(roundTrip).toEqual({
        classification: "Loop",
        generatedId: "camptownRacesUatReel-Loop",
        leadHasManualNote: true,
        sequence: "Intro, Verse, Chorus, CustomCue, Outro",
        settings: { pan: -0.2, transpose: 5, volume: 0.7 }
      });
  ...
  test("validates PR133-140 workspace payload handoff and launch split persistence", async ({ page }) => {
    const manifest = JSON.parse(await fs.readFile(uatManifestPath, "utf8"));
    const server = await openMidiStudioFromWorkspace(page, manifest);
    try {
      await expect(page.locator("body")).toHaveAttribute("data-midi-studio-launch-mode", "workspace");
      ...
      await selectMidiStudioTab(page, "song-setup");
      await page.locator("#songDetails [data-song-detail-field='classification']").fill("Boss");
      await fillSongSheetSectionBuilder(page, "Intro: C G\nVerse: G Am\nChorus: F C\nBridge:\nOutro: C");
      await addSongSheetSequenceLabels(page, ["Intro", "Verse", "Chorus", "Outro"]);
      await page.locator("#parseSongSheetButton").click();
      await selectMidiStudioTab(page, "instruments");
      await setInputValue(page, "#previewVolumeLeadInput", "0.65");
      await selectMidiStudioTab(page, "studio");
      await waitForCanvasRender(page);
      await timelineQuickInstrumentRow(page, "lead").locator("[data-quick-instrument-label='lead']").click();
      ...
      const workspaceState = await page.evaluate((expected) => {
        ...
        const contextSong = context.tools["midi-studio-v2"].songs.find((song) => song.id === context.tools["midi-studio-v2"].activeSongId);
        return {
          contextActiveSongId: context.tools["midi-studio-v2"].activeSongId,
          contextClassification: contextSong.classification,
          contextLeadHasNote: String(contextSong.studioArrangement.lanes.lead || "").includes(expected.rowToken),
          contextSequence: contextSong.studioArrangement.songSheet.sequence,
          dirty: session.dirty,
          sessionActiveSongId: session.data.activeSongId,
          sessionClassification: sessionSong.classification,
          sessionLeadHasNote: String(sessionSong.studioArrangement.lanes.lead || "").includes(expected.rowToken),
          sessionSequence: sessionSong.studioArrangement.songSheet.sequence,
          sessionVolume: sessionSong.studioArrangement.previewLaneSettings.volumes.lead
        };
      }, target);
      expect(workspaceState).toEqual(expect.objectContaining({
        contextActiveSongId: "camptownRacesUatReel-Boss",
        contextClassification: "Boss",
        contextLeadHasNote: true,
        contextSequence: "Intro, Verse, Chorus, Outro",
        sessionActiveSongId: "camptownRacesUatReel-Boss",
        sessionClassification: "Boss",
        sessionLeadHasNote: true,
        sessionSequence: "Intro, Verse, Chorus, Outro",
        sessionVolume: 0.65
      }));
      expect(workspaceState.dirty).toMatchObject({
        isDirty: true,
      ...
      expect(workspaceState.dirty.changedKeys).toEqual(expect.arrayContaining(["data.songs", "data.songs.studioArrangement", "data.songs.studioArrangement.previewLaneSettings"]));
      await page.locator("#returnToWorkspaceButton").click();
      await expect(page).toHaveURL(/workspace-manager-v2\/index\.html.*hostContextId=midi-studio-v2-workspace-context/);
      await expect(page).toHaveURL(/workspace=uat/);
    } finally {
  ...
  test("validates PR161-200 release candidate end-to-end UAT and visible control ownership", async ({ page }) => {
    test.slow();
    await page.setViewportSize({ width: 1600, height: 900 });
    const midiTabIds = ["song-setup", "studio", "instruments", "auto-create-parts", "midi-import", "diagnostics", "export"];
    const server = await openMidiStudioForImport(page);
    try {
      ...
      await expect(page.locator("#statusLog")).toHaveValue(/PASS Import JSON PASS: canonical payload loaded/);
      await selectMidiStudioTab(page, "song-setup");
      await page.locator("#songDetails [data-song-detail-field='name']").fill("Release Candidate Loop");
      await page.locator("#songDetails [data-song-detail-field='classification']").fill("Boss");
      await page.locator("#songDetailNotes [data-song-detail-field='notes']").fill("Release candidate UAT note.");
      await expect(page.locator("#songDetails [data-song-detail-field='id']")).toHaveValue("releaseCandidateLoop-Boss");
      await expect(page.locator("#songDetails [data-song-detail-field='id']")).toHaveJSProperty("readOnly", true);
      await expect(page.locator("[data-song-detail-help='classification']")).toHaveAttribute("title", /Menu.*Boss.*Chase/);
      await page.locator("#songSheetTempoInput").fill("156");
      await page.locator("#songSheetKeyInput").selectOption("D minor");
      await page.locator("#songSheetStyleInput").selectOption("chip");
      await page.locator("#gameUsageBossInput").setChecked(true);
      await page.locator("#gameUsageCustomInput").fill("Arena, Phase 2");
      await expect(page.locator("#gameUsageSummary")).toContainText("Boss, Arena, Phase 2");
      await expect(page.locator("#futureGameUsageRuntimeSyncButton")).toHaveAttribute("data-midi-studio-unwired", "not-implemented");

      await fillSongSheetSectionBuilder(page, "Intro:\nVerse:\nChorus:\nBridge:\nOutro:");
      await expect(page.locator("#songSheetAvailableSectionsList option[data-song-sheet-available-section]")).toHaveCount(0);
      await expect(page.locator("#songSheetAvailableSectionsList option[data-song-sheet-available-sections-empty='true']")).toHaveText("Populate a section to make it available.");
      await expect(page.locator("#songSheetSaveSectionButton")).toBeDisabled();
      await fillSongSheetSectionBuilder(page, "Intro: Dm A\nVerse: Dm Bb C A\nChorus: Bb F C Dm\nBridge: Gm Bb A\nOutro: Dm\nCustomSting: A Bb");
      await expect(page.locator("#songSheetAvailableSectionsList option")).toContainText([
        "Intro - 2 bars / 2 chords",
        "Verse - 4 bars / 4 chords",
        "Chorus - 4 bars / 4 chords",
        "Bridge - 3 bars / 3 chords",
        "Outro - 1 bar / 1 chord",
        "CustomSting - 2 bars / 2 chords"
      ]);

      await page.locator("#songSheetTemplateSectionSelect").selectOption("Chorus");
      await page.locator("#songSheetApplySectionTemplateButton").click();
      await expect(page.locator("#songSheetSectionChorusInput")).toHaveValue(/Bb F C Dm/);
      await page.locator("#songSheetAvailableSectionsList").selectOption("Intro");
      await page.locator("#songSheetSaveSectionButton").click();
      await expect(page.locator("#songSheetSectionLibrarySummary")).toContainText("Saved section asset: Intro");
      await page.locator("#songSheetLoadSectionButton").click();
      ...
      await page.locator("#songSheetSequenceList").evaluate((list) => {
        const option = document.createElement("option");
        option.value = "MissingBossCue";
        option.textContent = "MissingBossCue";
        list.append(option);
        list.selectedIndex = list.options.length - 1;
        list.dispatchEvent(new Event("change", { bubbles: true }));
      });
      await page.locator("#parseSongSheetButton").click();
      await expect(page.locator("#statusLog")).toHaveValue(/FAIL Song Sheet rejected: Song Sequence references missing or empty musical section: MissingBossCue/);
      await page.locator("#songSheetApplyArrangementTemplateButton").click();
      await page.locator("#songSheetApplyChordsPadInput").setChecked(true);
      await page.locator("#songSheetApplyBassInput").setChecked(true);
      await page.locator("#songSheetApplyDrumsInput").setChecked(true);
      await page.locator("#songSheetApplyLeadInput").setChecked(true);
      await page.locator("#parseSongSheetButton").click();
      await expect(page.locator("#statusLog")).toHaveValue(/OK Song Sheet parsed/);
      await expect(page.locator("#songSheetSummary [data-song-sheet-summary-field='sequence'] dd")).toHaveText("Intro, Verse, Chorus, Bridge, Chorus, Outro");
      await expect(page.locator("#songSheetSummary [data-song-sheet-summary-field='generation-targets'] dd")).toHaveText("Chords/Pad, Bass, Drums, Lead");
      await expect(page.locator("#instrumentGridSectionsInput")).toHaveValue(/Intro:2, Verse:4, Chorus:8, Bridge:3, Chorus:8, Outro:1/);
      await expect(page.locator("#inspectorOutput")).toContainText('"id": "releaseCandidateLoop-Boss"');
      ...
      expect(await hasCanvasNote(page, "lead", editTarget.rowToken, editTarget.stepIndex)).toBe(true);
      await clickCanvasCell(page, editTarget.rowToken, editTarget.stepIndex);
      expect(await hasCanvasNote(page, "lead", editTarget.rowToken, editTarget.stepIndex)).toBe(false);
      await dragCanvasCells(page, editTarget.rowToken, editTarget.stepIndex, editTarget.rowToken, editTarget.stepIndex + 2);
      for (let stepIndex = editTarget.stepIndex; stepIndex <= editTarget.stepIndex + 2; stepIndex += 1) {
        expect(await hasCanvasNote(page, "lead", editTarget.rowToken, stepIndex)).toBe(true);
      }
      ...
      expect((await canvasTimelineState(page)).frozenHeaderVisible).toBe(true);
      await page.locator("#playSequenceButton").click();
      await expect(page.locator("#instrumentGridTransportState")).toContainText("Playing sequence: Song Sequence");
      await page.locator("#stopTimingPreviewButton").click();
      ...
      await setInputValue(page, "#previewOctaveLowLeadInput", "4");
      await setInputValue(page, "#previewOctaveHighLeadInput", "6");
      await setInputValue(page, "#previewTransposeLeadInput", "3");
      await expect(page.locator("[data-audition-range-summary]")).toHaveText("C4 to B6");
      await page.locator("#instrumentAuditionKeyboard [data-audition-note='C5']").click();
      await expect(page.locator("#statusLog")).toHaveValue(/INFO Auditioned C5 for Lead/);
      await page.locator("#saveInstrumentPresetButton").click();
      await expect(page.locator("#instrumentPresetSummary")).toContainText("Saved instrument preset");
      ...
      await expect(page.locator("#songLibrarySummary")).toContainText("Saved song asset");
      await page.locator("#saveSongButton").click();
      await expect(page.locator("#songLibrarySummary")).toContainText(/Updated song asset|duplicate generated IDs are prevented/);
      await page.locator("#duplicateSongButton").click();
      const idsAfterDuplicate = await page.evaluate(() => window.__midiStudioV2App.payload.songs.map((song) => song.id));
      expect(new Set(idsAfterDuplicate).size).toBe(idsAfterDuplicate.length);
      await page.evaluate(() => window.__midiStudioV2App.selectSong("releaseCandidateLoop-Boss"));
      await expect(page.locator("#nowPlayingLabel")).toHaveText("Selected: Release Candidate Loop");

      await selectMidiStudioTab(page, "midi-import");
      await expect(page.locator("#midiSourceDetails")).toContainText("Local MIDI file import is separate from JSON manifest import");
      await expect(page.locator("#midiSourceDetails")).not.toContainText("missing MIDI path");
      await page.locator("#midiSourceFileInput").setInputFiles({
        buffer: corruptMidiBytes,
        mimeType: "audio/midi",
        name: "release-candidate-broken.midi"
      });
      await expect(page.locator("#midiSourceDetails")).toContainText("FAIL MIDI source validation failed for release-candidate-broken.midi");
      await expect(page.locator("#statusLog")).toHaveValue(/FAIL MIDI source validation failed for release-candidate-broken\.midi/);
      await expect(page.locator("#futureMidiCanonicalConversionButton")).toHaveAttribute("data-midi-studio-unwired", "not-implemented");

      await selectMidiStudioTab(page, "diagnostics");
      await expect(page.locator("#inspectorOutput")).toContainText('"id": "releaseCandidateLoop-Boss"');
      await expect(page.locator("#diagnosticsManifestReadinessDetails [data-export-field='game-usage-assignment'] dd")).toContainText("Boss: Release Candidate Loop");
      const diagnosticEditableControls = await page.locator('[data-midi-studio-tab-panel="diagnostics"] input:not([type="hidden"]):not([readonly]), [data-midi-studio-tab-panel="diagnostics"] select:not([disabled]), [data-midi-studio-tab-panel="diagnostics"] textarea:not([readonly])').count();
      expect(diagnosticEditableControls).toBe(0);
      await page.locator("#toolCopyJsonButton").click();
      await expect(page.locator("#statusLog")).toHaveValue(/MIDI Studio V2 toolState/);
      ...
      for (const [format, label] of [["wav", "Save WAV"], ["mp3", "Save MP3"], ["ogg", "Save OGG"]]) {
        await page.locator("#renderedExportTargetTypeSelect").selectOption(format);
      ...
      await expect(page.locator("#statusLog")).toHaveValue(/PASS Export JSON PASS/);
      const exportedToolState = JSON.parse(await page.locator("#inspectorOutput").textContent());
      const exportedSong = exportedToolState.payload.songs.find((song) => song.id === "releaseCandidateLoop-Boss");
      expect(exportedSong).toEqual(expect.objectContaining({
        classification: "Boss",
        id: "releaseCandidateLoop-Boss",
        name: "Release Candidate Loop"
      }));
      expect(exportedSong.director.usage).toEqual(expect.arrayContaining(["Boss", "Arena", "Phase 2"]));
      expect(exportedSong.studioArrangement.previewLaneSettings.volumes.lead).toBe(0.7);
      expect(exportedSong.studioArrangement.previewLaneSettings.pans.lead).toBe(-0.2);
      expect(exportedSong.studioArrangement.previewLaneSettings.transposes.lead).toBe(3);
      ...
      await page.locator("#toolImportManifestInput").setInputFiles({
        buffer: Buffer.from(JSON.stringify(exportedToolState)),
        mimeType: "application/json",
        name: "pr161-200-roundtrip.tool-state.json"
      });
      await expect(page.locator("#statusLog")).toHaveValue(/via toolState\.payload/);
      expect(await page.evaluate(() => ({
        localToolState: window.localStorage.getItem("workspace.tools.midi-studio-v2"),
        sessionToolState: window.sessionStorage.getItem("workspace.tools.midi-studio-v2"),
        song: window.__midiStudioV2App.selectedSong()
      }))).toMatchObject({
        localToolState: null,
      ...
      const controls = await expectMidiStudioReleaseCandidateControlAudit(page, midiTabIds);
      expect(controls.filter((control) => control.wired === "unwired").length).toBeGreaterThan(0);
      await expectMidiStudioAccordionAudit(page, midiTabIds);

      await selectMidiStudioTab(page, "studio");
      await page.locator("#instrumentGridSectionSelect").selectOption("Outro");
      await page.locator("#loopToggle").setChecked(false);
      await page.locator("#playButton").click();
      await expect(page.locator("#playbackState")).toContainText("Playing audible preview");
      await expect.poll(async () => (await page.locator("#playbackState").textContent()) || "", { timeout: 10000 }).toContain("Completed audible preview");
      await expect(page.locator("#playButton")).toBeEnabled();
      await page.locator("#loopToggle").setChecked(true);
      await page.locator("#playButton").click();
      await expect(page.locator("#playbackState")).toContainText("(looping)");
  ...
  test("validates PR161-200 release candidate workspace handoff and tool launch split", async ({ page }) => {
    const manifest = JSON.parse(await fs.readFile(uatManifestPath, "utf8"));
    let server = await openMidiStudioFromWorkspace(page, manifest);
    try {
      await expect(page.locator("body")).toHaveAttribute("data-midi-studio-launch-mode", "workspace");
      ...
      await expect(page.locator("#toolExportToolStateButton")).toBeHidden();
      await selectMidiStudioTab(page, "song-setup");
      await page.locator("#songDetails [data-song-detail-field='name']").fill("Workspace Release Cue");
      await page.locator("#songDetails [data-song-detail-field='classification']").fill("Victory");
      await fillSongSheetSectionBuilder(page, "Intro: C G\nVerse: G Am\nChorus: F C\nBridge: Dm G\nOutro: C");
      await addSongSheetSequenceLabels(page, ["Intro", "Verse", "Chorus", "Outro"]);
      await page.locator("#parseSongSheetButton").click();
      await selectMidiStudioTab(page, "studio");
      ...
      expect(workspaceState).toEqual(expect.objectContaining({
        contextGeneratedId: "workspaceReleaseCue-Victory",
        contextLeadHasNote: true,
        sessionGeneratedId: "workspaceReleaseCue-Victory",
        sessionLeadHasNote: true
      }));
      expect(workspaceState.dirty).toMatchObject({ isDirty: true });
    } finally {
  ...
  test("validates PR501-560 production signoff workflow, readiness, and SSoT", async ({ page }) => {
    await page.setViewportSize({ width: 1600, height: 900 });
    const server = await openMidiStudioForImport(page);
    try {
      await page.locator("#toolImportManifestInput").setInputFiles(uatManifestPath);
      await selectMidiStudioTab(page, "song-setup");
      await page.locator("#songDetails [data-song-detail-field='name']").fill("Production Signoff Cue");
      await page.locator("#songDetails [data-song-detail-field='classification']").fill("Victory");
      await page.locator("#songSheetTempoInput").fill("960");
      await page.locator("#songSheetKeyInput").selectOption("C major");
      await page.locator("#songSheetStyleInput").selectOption("victory-fanfare");
      await page.locator("#songDetailNotes [data-song-detail-field='notes']").fill("Production signoff round-trip note.");
      await expect(page.locator("#songDetails [data-song-detail-field='id']")).toHaveValue("productionSignoffCue-Victory");
      ...
      await selectMidiStudioTab(page, "studio");
      await waitForCanvasRender(page);
      await expect(octaveTimelineCanvas(page)).toHaveAttribute("data-section-header-labels", "Intro|Verse|Chorus|Outro");
      await timelineQuickInstrumentRow(page, "lead").locator("[data-quick-instrument-label='lead']").click();
      const editTarget = await emptyCanvasRun(page, { lane: "lead", length: 1 });
      expect(editTarget).toBeTruthy();
      await clickCanvasCell(page, editTarget.rowToken, editTarget.stepIndex);
      ...
      await setInputValue(page, "#previewVolumeLeadInput", "0.7");
      await setInputValue(page, "#previewPanLeadInput", "-0.2");
      await setInputValue(page, "#previewTransposeLeadInput", "4");
      await setInputValue(page, "#previewVelocityLeadInput", "92");
      await setInputValue(page, "#previewDurationLeadInput", "1.25");
      await setInputValue(page, "#previewEffectReverbLeadInput", "0.4");
      await setInputValue(page, "#previewEffectChorusLeadInput", "0.25");
      await selectMidiStudioTab(page, "studio");
      await waitForCanvasRender(page);
      await page.locator("#duplicateInstrumentRowButton").click();
      await expect(page.locator("#statusLog")).toHaveValue(/OK Duplicated instrument row Lead/);
      await page.locator("#moveInstrumentDownButton").click();
      await expect(page.locator("#statusLog")).toHaveValue(/OK Moved instrument row/);
      await selectMidiStudioTab(page, "instruments");
      await instrumentRow(page, "lead-1").locator("[data-delete-instrument-row='lead-1']").click();
      await expect(page.locator("#statusLog")).toHaveValue(/WARN Confirm delete requested/);
      await page.locator("[data-confirm-delete-instrument-row='lead-1']").click();
      await expect(page.locator("#statusLog")).toHaveValue(/OK Deleted instrument row/);
      await selectInstrumentRow(page, "lead");
      ...
      await selectMidiStudioTab(page, "export");
      await expect(page.locator("#exportManifestReadinessDetails")).not.toContainText("Missing assignments WARN");
      await expect(page.locator("#exportManifestReadinessDetails")).toContainText("Optional unassigned songs");
      await expect(page.locator("#exportStatusDetails")).toContainText("Save WAV renders from the SoundFont pipeline");
      await expect(page.locator("#renderedExportTargetTypeSelect option")).toContainText([
        "WAV",
        "MP3 (encoder unavailable)",
        "OGG (encoder unavailable)"
      ]);
      ...
      await selectMidiStudioTab(page, "midi-import");
      await expect(page.locator("#midiSourceDetails")).toContainText("INFO No MIDI source inspected yet.");
      await expect(page.locator("#midiSourceDetails")).not.toContainText("WARN No MIDI source inspected");
      await page.locator("#midiSourceFileInput").setInputFiles({
        buffer: validMidiBytes,
        mimeType: "audio/midi",
        name: "production-signoff.midi"
      });
      await expect(page.locator("#midiSourceDetails")).toContainText("PASS Local MIDI notes can populate the canonical studio arrangement");

      await page.evaluate(() => window.__midiStudioV2App.selectSong("productionSignoffCue-Victory"));
      await selectMidiStudioTab(page, "export");
      await page.evaluate(() => {
        window.__midiStudioRenderedDownloads = [];
        window.__midiStudioV2App.triggerRenderedDownload = (blob, filename) => {
      ...
      await page.locator("#renderedExportTargetTypeSelect").selectOption("wav");
      await page.locator("#renderedExportSaveButton").click();
      await expect.poll(() => page.evaluate(() => window.__midiStudioRenderedDownloads[0]?.type || "")).toBe("audio/wav");
      await page.locator("#renderedExportTargetTypeSelect").selectOption("mp3");
      await expect(page.locator("#renderedExportSaveButton")).toHaveAttribute("data-midi-studio-unwired", "encoder-unavailable");
      await page.locator("#renderedExportTargetTypeSelect").selectOption("ogg");
      await expect(page.locator("#renderedExportSaveButton")).toHaveAttribute("data-midi-studio-unwired", "encoder-unavailable");
      ...
      await page.locator("#toolImportManifestInput").setInputFiles({
        buffer: Buffer.from(exportedJson),
        mimeType: "application/json",
        name: "midi-studio-v2-pr501-roundtrip.json"
      });
      expect(await page.evaluate(() => {
        const song = window.__midiStudioV2App.selectedSong();
        return {
          effects: song.studioArrangement.previewLaneSettings.effects.lead,
          id: song.id,
          pan: song.studioArrangement.previewLaneSettings.pans.lead,
          transpose: song.studioArrangement.previewLaneSettings.transposes.lead,
          volume: song.studioArrangement.previewLaneSettings.volumes.lead
        };
      })).toEqual(expect.objectContaining({
        id: "productionSignoffCue-Victory",
        pan: -0.2,
        transpose: 4,
        volume: 0.7
      }));
      ...
      const controls = [];
      for (const tabId of ["song-setup", "studio", "instruments", "midi-import", "diagnostics", "export"]) {
        await selectMidiStudioTab(page, tabId);
        if (tabId === "studio") {
          await waitForCanvasRender(page);
        }
      ...
      const duplicateEditableOwners = Array.from(controls
        .filter((control) => control.editable && control.kind === "canonical")
        .reduce((map, control) => {
          const owners = map.get(control.canonical) || new Set();
          owners.add(control.owner);
          map.set(control.canonical, owners);
          return map;
        }, new Map()), ([canonical, owners]) => ({ canonical, owners: Array.from(owners) }))
        .filter((entry) => entry.owners.length > 1);
      expect(duplicateEditableOwners).toEqual([]);
      expect(controls.filter((control) => control.unwired && (!control.unwiredClass || !control.title))).toEqual([]);
    } finally {
  ...
  test("validates PR501-560 workspace launch handoff split", async ({ page }) => {
    await page.setViewportSize({ width: 1600, height: 900 });
    const manifest = JSON.parse(await fs.readFile(uatManifestPath, "utf8"));
    const server = await openMidiStudioFromWorkspace(page, manifest);
    try {
      ...
      await waitForCanvasRender(page);
      await timelineQuickInstrumentRow(page, "lead").locator("[data-quick-instrument-label='lead']").click();
      const workspaceEdit = await emptyCanvasRun(page, { lane: "lead", length: 1 });
      expect(workspaceEdit).toBeTruthy();
      await clickCanvasCell(page, workspaceEdit.rowToken, workspaceEdit.stepIndex);
      expect(await page.evaluate(() => {
        const session = JSON.parse(window.sessionStorage.getItem("workspace.tools.midi-studio-v2"));
