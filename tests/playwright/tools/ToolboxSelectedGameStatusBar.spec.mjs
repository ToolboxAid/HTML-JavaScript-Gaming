...
async function statusBarSnapshot(page) {
  return page.locator("[data-toolbox-status-bar]").evaluate((bar) => {
    ...
    const centerPanelStyle = centerPanel ? getComputedStyle(centerPanel) : null;
    const statusHeightReserve = parseFloat(getComputedStyle(document.body).getPropertyValue("--toolbox-status-bar-height")) || 0;
    const barBox = bar.getBoundingClientRect();
    return {
      bottomGap: Math.round(window.innerHeight - barBox.bottom),
      barBox: boxSnapshot(bar),
      centerPanelBox: boxSnapshot(centerPanel),
      centerPanelBottomReserve: centerPanelStyle
        ? Math.max(parseFloat(centerPanelStyle.paddingBottom) || 0, parseFloat(centerPanelStyle.marginBottom) || 0)
        : 0,
      dataset: { ...bar.dataset },
...
test("shared toolbox status bar anchors to the bottom in tool display mode", async ({ page }) => {
  ...
  try {
    ...
    expect(Math.abs(snapshot.centerPanelBottomReserve - snapshot.statusHeightReserve)).toBeLessThanOrEqual(1);
