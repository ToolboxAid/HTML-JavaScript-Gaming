...
async function armInputMappingCapture(page, source, gestureBinding, { gamepadIndex = 0 } = {}) {
  const captureSelector = inputMappingCaptureSelector(source, gamepadIndex);
  const captureButton = page.locator(captureSelector);
  const gestureButton = page.locator(`.input-mapping-v2__gesture-button[data-input-mapping-gesture-binding='${gestureBinding}']`);
  await expect(captureButton).not.toHaveClass(/is-capture-(complete|canceled|warning)/, { timeout: 1200 }).catch(() => {});
  const isRequestedCaptureSelected = await captureButton.evaluate((button) => (
    button.classList.contains("is-selected-capture-device")
  )).catch(() => false);
  if (!isRequestedCaptureSelected) {
  ...
  if (source === "gamepad") {
    await expect(captureButton).toHaveClass(/is-capturing/, { timeout: 1200 }).catch(() => {});
    await page.waitForTimeout(150);
  }
}

function inputMappingCaptureSelector(source, gamepadIndex = 0) {
  if (source === "keyboard") {
    return "#inputMappingV2CaptureKeyboardButton";
  }
  if (source === "mouse") {
    return "#inputMappingV2CaptureMouseButton";
  }
  return `.input-mapping-v2__gamepad-capture-button[data-input-mapping-gamepad-index='${gamepadIndex}']`;
}
...
async function expectWorkspaceToolsDisabled(page) {
  await expect(page.locator("#workspaceToolTiles [data-workspace-tool-id]")).toHaveCount(12);
...
async function expectWorkspaceReturnRehydrated(page, { gameId = "Asteroids", repoName = "HTML-JavaScript-Gaming" } = {}) {
  ...
  await expect(page.locator('[data-workspace-tool-id="audio-sfx-playground-v2"]')).toBeEnabled();
  ...
  await expect(page.locator('[data-workspace-tool-id="midi-studio-v2"]')).toBeEnabled();
...
async function expectWorkspaceRestoreRequiresRepoRebind(page, { dirty = false, gameId = "Asteroids", repoName = "HTML-JavaScript-Gaming" } = {}) {
  ...
  await expect(page.locator('[data-workspace-tool-id="audio-sfx-playground-v2"]')).toBeDisabled();
  ...
  await expect(page.locator('[data-workspace-tool-id="midi-studio-v2"]')).toBeDisabled();
...
async function expectWorkspaceReturnedFromTool(page, options = {}) {
  ...
  const enabledToolCount = options.enabledToolCount ?? (gameId === "Asteroids" ? 11 : 9);
  await expectWorkspaceReturnRehydrated(page, { gameId, repoName });
  await expect(page.locator("#saveWorkspaceButton"))[dirty ? "toBeEnabled" : "toBeDisabled"]();
  await expect(page.locator("#closeWorkspaceButton"))[dirty ? "toBeDisabled" : "toBeEnabled"]();
  await expect(page.locator("#cancelWorkspaceButton"))[dirty ? "toBeEnabled" : "toBeDisabled"]();
...
test.describe("Workspace Manager V2 bootstrap", () => {
  ...
  test("navigation toolbox accordion menu shows Toolbox IA grouping", async ({ page }) => {
    const pageErrors = [];
    ...
    try {
      ...
      expect(headerToolboxMenu.accordions).toEqual([
        {
          label: "Objects",
        ...
        },
        {
          label: "Colors",
          items: ["Colors"],
          routes: ["palette-manager"],
          open: false
        },
        {
          label: "Assets",
          items: ["Assets"],
          routes: ["storage-inspector"],
          open: false
        }
      ]);
      const accordionToggleState = await page.locator("[data-toolbox-menu]").evaluate((menu) => {
        const objectsAccordion = menu.querySelector("[data-toolbox-menu-accordion]");
        const summary = objectsAccordion.querySelector("summary");
        const beforeOpen = objectsAccordion.open;
        summary.click();
        const afterOpen = objectsAccordion.open;
        summary.click();
        const afterClose = objectsAccordion.open;
        return { beforeOpen, afterOpen, afterClose };
      });

      expect(accordionToggleState).toEqual({ beforeOpen: false, afterOpen: true, afterClose: false });
  ...
  test("auto-polls Input Mapping V2 gamepads on load", async ({ page }) => {
    ...
    try {
      ...
      const captureBottom = await page.locator("#captureInputContent").evaluate((content) => {
        ...
        return {
          lastElementId: content.lastElementChild?.id,
          separatorClass: refresh?.previousElementSibling?.className,
  ...
  test("keeps Input Mapping V2 capture sessions and active input visuals engine-backed", async () => {
    ...
    expect(inputServiceSource).toContain("beginComboCapture(options = {})");
    expect(inputServiceSource).toContain("recordComboInput(input, options = {})");
  ...
  test("keeps Input Mapping V2 spacing compact and highlights saved selected inputs", async ({ page }) => {
    ...
    try {
      ...
      await armInputMappingCapture(page, "keyboard", "KeyboardPress");
      ...
      await expect(page.locator("#inputMappingV2UsedInputHighlights")).toHaveCount(0);

      await page.locator("#inputMappingV2ActionSelect").selectOption("confirm");
      await page.locator("#inputMappingV2AddActionButton").click();
      await expect(page.locator("#inputMappingV2CaptureKeyboardButton")).not.toHaveClass(/has-used-input/);
  ...
  test("selects Input Mapping V2 gestures for capture and highlights used controls", async ({ page }) => {
    ...
    try {
      ...
      await armInputMappingCapture(page, "keyboard", "KeyboardRelease");
      ...
      await page.locator("#inputMappingV2AddActionButton").click();
      await armInputMappingCapture(page, "mouse", "MouseClick");
      ...
      await page.locator("#inputMappingV2AddActionButton").click();
      await armInputMappingCapture(page, "gamepad", "GameControllerButtonPress");
      await expect(page.locator(".input-mapping-v2__gesture-button[data-input-mapping-gesture-binding='GameControllerButtonPress']")).toHaveClass(/is-selected/);
      await page.evaluate(() => {
      ...
      await expect(page.locator(".input-mapping-v2__mapping-card[data-input-mapping-tile-action-id='start']")).toContainText("GD, Button 1, Btn Press");
      await expect(page.locator(".input-mapping-v2__mapping-card[data-input-mapping-tile-action-id='start'] .input-mapping-v2__input-token[data-input-mapping-binding='Pad0:Button1:GameControllerButtonPress']")).toHaveClass(/is-selected-mapping-input/);
      ...
      await armInputMappingCapture(page, "gamepad", "GameControllerCombo");
      ...
      await expect(page.locator(".input-mapping-v2__mapping-card[data-input-mapping-tile-action-id='start']")).not.toContainText("Combo, GD Button 1 + Alt");
      ...
      await expect(page.locator(".input-mapping-v2__mapping-card[data-input-mapping-tile-action-id='start']")).toContainText("Combo, GD Button 1 + Alt");
  ...
  test("sizes Input Mapping V2 columns and live-highlights mapped non-keyboard inputs", async ({ page }) => {
    ...
    try {
      ...
      const columnSizing = await page.locator(".input-mapping-v2.tool-starter.app-shell").evaluate((shell) => {
        ...
        const capture = centerAccordions[0];
        const gestures = centerAccordions[1];
      ...
      await armInputMappingCapture(page, "mouse", "MouseWheelUp");
      await page.evaluate(() => {
      ...
      await expect(cancelTile).toContainText("GD, Button 1, Btn Press");
      ...
      const tokenLayout = await cancelTile.locator(".input-mapping-v2__input-token").evaluateAll((tokens) => tokens.map((token) => ({
        ...
        flexBasis: getComputedStyle(token).flexBasis,
      ...
      expect(tokenLayout.every((entry) => entry.cardWidth === 250)).toBe(true);
      expect(tokenLayout.every((entry) => entry.flexBasis === "100%")).toBe(true);
      expect(tokenLayout.every((entry) => entry.width < entry.cardWidth && entry.width > 200)).toBe(true);
      ...
      const gamepadToken = cancelTile.locator(".input-mapping-v2__input-token[data-input-mapping-binding='Pad0:Button1:GameControllerButtonPress']");
  ...
  test("live-highlights Input Mapping V2 release, combo, and double-click mapping tokens", async ({ page }) => {
    ...
    try {
      ...
      await page.locator("#inputMappingV2ActionSelect").selectOption("confirm");
      await page.locator("#inputMappingV2AddActionButton").click();
      await armInputMappingCapture(page, "keyboard", "KeyboardCombo");
      await page.evaluate(() => {
        window.dispatchEvent(new KeyboardEvent("keydown", {
          bubbles: true,
          cancelable: true,
      ...
      await page.locator("#inputMappingV2ActionSelect").selectOption("fire");
      await page.locator("#inputMappingV2AddActionButton").click();
      await armInputMappingCapture(page, "mouse", "MouseDoubleClick");
      await page.evaluate(() => {
        window.dispatchEvent(new MouseEvent("mousedown", {
          bubbles: true,
          button: 0,
  ...
  test("renders Input Mapping V2 tokens and gates capture by selected gesture", async ({ page }) => {
    ...
    try {
      ...
      const gestureDisabledState = async () => page.locator("#inputMappingV2GestureList").evaluate((content) => ({
        gamepad: content.querySelector(".input-mapping-v2__gesture-button[data-input-mapping-gesture-binding='GameControllerStick']").disabled,
        keyboard: content.querySelector(".input-mapping-v2__gesture-button[data-input-mapping-gesture-binding='KeyboardHold']").disabled,
        mouse: content.querySelector(".input-mapping-v2__gesture-button[data-input-mapping-gesture-binding='MouseClick']").disabled
      }));
      ...
      await page.locator("#inputMappingV2CaptureKeyboardButton").click();
      await expect(page.locator("#inputMappingV2CaptureKeyboardButton")).toHaveClass(/is-selected-capture-device/);
      expect(await gestureDisabledState()).toEqual({
        gamepad: true,
        keyboard: false,
        mouse: true
      });
      await page.locator(".input-mapping-v2__gesture-group", { hasText: "Keyboard" })
        .locator(".input-mapping-v2__gesture-button[data-input-mapping-gesture-binding='KeyboardHold']")
        .click();
      ...
      await expect(page.locator("#inputMappingV2CaptureMouseButton")).toHaveClass(/is-selected-capture-device/);
      expect(await gestureDisabledState()).toEqual({
        gamepad: true,
        keyboard: true,
        mouse: false
      ...
      await expect(page.locator(".input-mapping-v2__gamepad-capture-button[data-input-mapping-gamepad-index='0']")).toHaveClass(/is-selected-capture-device/);
      expect(await gestureDisabledState()).toEqual({
        gamepad: false,
        keyboard: true,
        mouse: true
      });
      await expect(page.locator(".input-mapping-v2__gesture-button[data-input-mapping-gesture-binding='GameControllerButton']")).toHaveCount(0);
      await page.locator(".input-mapping-v2__gesture-group", { hasText: "Game Controller" })
        .locator(".input-mapping-v2__gesture-button[data-input-mapping-gesture-binding='GameControllerStick']")
        .click();
      await page.evaluate(() => {
        window.__inputMappingV2MockGamepads[0] = {
          ...window.__inputMappingV2MockGamepads[0],
      ...
      await expect(fireTile.locator(".input-mapping-v2__input-separator")).toHaveCount(0);
      const tokenStructure = await fireTile.locator(".input-mapping-v2__token-list").evaluate((list) => ({
        elementClasses: Array.from(list.children).map((child) => child.className),
        textNodes: Array.from(list.childNodes)
      ...
      await expect(page.locator(".input-mapping-v2__input-token[data-input-mapping-binding='KeyH:KeyboardHold']")).toHaveText("Keyboard, KeyH, Hold");
      await expect(page.locator(".input-mapping-v2__input-token[data-input-mapping-binding='MouseButton0']")).toHaveText("Mouse, Left Button, Click");
      await expect(page.locator(".input-mapping-v2__input-token[data-input-mapping-binding='Pad0:Axis0+']")).toHaveText("GD, Axis 0+, Stick");
      ...
      await expect(fireTile.locator(".input-mapping-v2__input-token[data-input-mapping-binding='Pad0:Axis0+']")).toHaveAttribute("title", /Arcade Flight Stick[\s\S]*Axis 0\+/);
      ...
      expect(tokenMetrics.every((token) => token.cardWidth === 250)).toBe(true);
      expect(tokenMetrics.every((token) => token.flexBasis === "100%")).toBe(true);
      expect(tokenMetrics.every((token) => token.width < token.cardWidth && token.width > 200)).toBe(true);
  ...
  test("captures two-input combos and preserves Input Mapping V2 tile scroll during token updates", async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator("#inputMappingV2UsedInputHighlights")).toHaveCount(0);
      await page.locator("#inputMappingV2ActionSelect").selectOption("fire");
      await expect(page.locator("#inputMappingV2CaptureKeyboardButton")).toBeDisabled();
      await expect(page.locator("#gestureSetupContent .input-mapping-v2__gesture-button").first()).toBeDisabled();
      await expect(page.locator(".input-mapping-v2__mapping-card")).toHaveCount(0);
      await page.locator("#inputMappingV2AddActionButton").click();
      await expect(page.locator(".input-mapping-v2__mapping-card[data-input-mapping-tile-action-id='fire']")).toHaveCount(1);
      await expect(page.locator(".input-mapping-v2__gesture-button[data-input-mapping-gesture-binding='KeyboardPress']")).toBeDisabled();
      ...
      const overflowState = await fireTile.evaluate((card) => ({
        ...
        tokenFlexBasis: Array.from(card.querySelectorAll(".input-mapping-v2__input-token"))
          .map((token) => getComputedStyle(token).flexBasis),
      ...
      expect(overflowState.cardWidth).toBe(250);
      expect(overflowState.overflowY).toBe("auto");
      expect(overflowState.scrollHeight).toBeGreaterThan(overflowState.clientHeight);
      expect(overflowState.tokenFlexBasis.every((flexBasis) => flexBasis === "100%")).toBe(true);
      expect(overflowState.tokenWidths.every((width) => width < overflowState.cardWidth && width > 200)).toBe(true);
      ...
      await expect(page.locator("#inspectorOutput")).not.toContainText('"binding": "Combo:ControlLeft+KeyS"');
      await page.evaluate(() => {
        window.dispatchEvent(new KeyboardEvent("keydown", {
          bubbles: true,
          cancelable: true,
      ...
      const comboToken = fireTile.locator(".input-mapping-v2__input-token[data-input-mapping-binding='Combo:ControlLeft+KeyS']");
      await expect(comboToken).toHaveText("Combo, Ctrl + S");
      await expect(page.locator("#inspectorOutput")).toContainText('"binding": "Combo:ControlLeft+KeyS"');
      await expect.poll(() => fireTile.evaluate((card) => card.scrollTop)).toBe(selectedScrollTop);
      ...
      await expect(page.locator("#statusLog")).toHaveValue(/OK Deleted Combo Ctrl \+ S from Fire\./);
      await expect.poll(() => fireTile.evaluate((card) => card.scrollTop)).toBe(selectedScrollTop);
  ...
  test("honors Input Mapping V2 gesture-specific capture sessions", async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator("#statusLog")).toHaveValue(/WARN Mouse Wheel Up capture expects wheel input; mouse click ignored\./);
      await expect(page.locator(".input-mapping-v2__input-token[data-input-mapping-binding='MouseWheelUp']")).toHaveCount(0);
      await page.evaluate(() => {
        window.dispatchEvent(new WheelEvent("wheel", {
          bubbles: true,
          cancelable: true,
      ...
      await expect(page.locator("#statusLog")).toHaveValue(/WARN Mouse Wheel Up capture expects Wheel Up; Wheel Down was ignored\./);
      await expect(page.locator(".input-mapping-v2__input-token[data-input-mapping-binding='MouseWheelUp']")).toHaveCount(0);
      await page.evaluate(() => {
        window.dispatchEvent(new WheelEvent("wheel", {
          bubbles: true,
      ...
      await expect(page.locator(".input-mapping-v2__input-token[data-input-mapping-binding='MouseWheelUp']")).toHaveText("Mouse, Wheel Up, Wheel");
      await expect(page.locator("#inputMappingV2CaptureMouseButton")).not.toHaveClass(/is-capturing/);

      await page.locator("#inputMappingV2ActionSelect").selectOption("thrust");
      await page.locator("#inputMappingV2AddActionButton").click();
      await armInputMappingCapture(page, "mouse", "MouseWheelDown");
      await page.evaluate(() => {
        window.dispatchEvent(new WheelEvent("wheel", {
          bubbles: true,
          cancelable: true,
      ...
      await expect(page.locator(".input-mapping-v2__input-token[data-input-mapping-binding='MouseWheelDown']")).toHaveText("Mouse, Wheel Down, Wheel");
      await expect(page.locator("#inputMappingV2CaptureMouseButton")).not.toHaveClass(/is-capturing/);

      await page.locator("#inputMappingV2ActionSelect").selectOption("menu");
      await page.locator("#inputMappingV2AddActionButton").click();
      await armInputMappingCapture(page, "gamepad", "GameControllerTrigger");
      await page.evaluate(() => {
        window.__inputMappingV2MockGamepads[0] = {
          ...window.__inputMappingV2MockGamepads[0],
          buttons: window.__inputMappingV2MockGamepads[0].buttons.map((button, index) => ({ pressed: index === 0 })),
          timestamp: 71
        };
      });
      await expect(page.locator("#statusLog")).toHaveValue(/WARN Game Controller Trigger capture expects trigger input; A is button input\./, { timeout: 2500 });
      await expect(page.locator(".input-mapping-v2__mapping-card[data-input-mapping-tile-action-id='menu'] .input-mapping-v2__input-token")).toHaveCount(0);
      await page.evaluate(() => {
        window.__inputMappingV2MockGamepads[0] = {
          ...window.__inputMappingV2MockGamepads[0],
          buttons: window.__inputMappingV2MockGamepads[0].buttons.map((button, index) => ({ pressed: index === 6 })),
          timestamp: 72
        };
      });
      await expect(page.locator(".input-mapping-v2__input-token[data-input-mapping-binding='Pad0:Button6']")).toHaveText("GD, LT, Trigger", { timeout: 2500 });
      ...
      await expect(page.locator("#statusLog")).toHaveValue(/WARN Game Controller Btn Press capture expects button press input; LT is trigger input\./, { timeout: 2500 });
      await expect(page.locator(".input-mapping-v2__mapping-card[data-input-mapping-tile-action-id='start'] .input-mapping-v2__input-token")).toHaveCount(0);
      await page.evaluate(() => {
        window.__inputMappingV2MockGamepads[0] = {
          ...window.__inputMappingV2MockGamepads[0],
          buttons: window.__inputMappingV2MockGamepads[0].buttons.map((button, index) => ({ pressed: index === 1 })),
          timestamp: 75
        };
      });
      await expect(page.locator(".input-mapping-v2__input-token[data-input-mapping-binding='Pad0:Button1:GameControllerButtonPress']")).toHaveText("GD, B, Btn Press", { timeout: 2500 });
      await expect(page.locator(".input-mapping-v2__gamepad-capture-button[data-input-mapping-gamepad-index='0']")).not.toHaveClass(/is-capturing/);
      const standardButtonToken = page.locator(".input-mapping-v2__input-token[data-input-mapping-binding='Pad0:Button1:GameControllerButtonPress']");
      ...
      await expect(standardButtonToken).not.toHaveClass(/is-action-active/, { timeout: 2500 });
      await page.locator("#inputMappingV2ActionSelect").selectOption("moveRight");
      await page.locator("#inputMappingV2AddActionButton").click();
      await armInputMappingCapture(page, "gamepad", "GameControllerButtonHold");
      await page.evaluate(() => {
        window.__inputMappingV2MockGamepads[0] = {
          ...window.__inputMappingV2MockGamepads[0],
          buttons: window.__inputMappingV2MockGamepads[0].buttons.map((button, index) => ({ pressed: index === 3 })),
          timestamp: 76.5
        };
      });
      await expect(page.locator(".input-mapping-v2__input-token[data-input-mapping-binding='Pad0:Button3:GameControllerButtonHold']")).toHaveText("GD, Y, Btn Hold", { timeout: 2500 });
      await expect(page.locator(".input-mapping-v2__gamepad-capture-button[data-input-mapping-gamepad-index='0']")).not.toHaveClass(/is-capturing/);
      await page.evaluate(() => {
        window.__inputMappingV2MockGamepads[0] = {
          ...window.__inputMappingV2MockGamepads[0],
      ...
      await page.waitForTimeout(150);
      await page.locator("#inputMappingV2ActionSelect").selectOption("select");
      await page.locator("#inputMappingV2AddActionButton").click();
      await armInputMappingCapture(page, "gamepad", "GameControllerButtonRelease");
      await page.evaluate(() => {
        window.__inputMappingV2MockGamepads[0] = {
          ...window.__inputMappingV2MockGamepads[0],
      ...
      await expect(page.locator(".input-mapping-v2__mapping-card[data-input-mapping-tile-action-id='select'] .input-mapping-v2__input-token")).toHaveCount(0);
      await page.evaluate(() => {
        window.__inputMappingV2MockGamepads[0] = {
          ...window.__inputMappingV2MockGamepads[0],
          buttons: window.__inputMappingV2MockGamepads[0].buttons.map(() => ({ pressed: false })),
      ...
      await page.locator("#inputMappingV2AddActionButton").click();
      await armInputMappingCapture(page, "gamepad", "GameControllerDPad");
      await page.evaluate(() => {
        window.__inputMappingV2MockGamepads[0] = {
          ...window.__inputMappingV2MockGamepads[0],
          buttons: window.__inputMappingV2MockGamepads[0].buttons.map((button, index) => ({ pressed: index === 14 })),
          timestamp: 80
        };
      });
      await expect(page.locator(".input-mapping-v2__input-token[data-input-mapping-binding='Pad0:Button14']")).toHaveText("GD, DPad Left, DPad", { timeout: 2500 });
      await expect(page.locator(".input-mapping-v2__gamepad-capture-button[data-input-mapping-gamepad-index='0']")).not.toHaveClass(/is-capturing/);
      expect(pageErrors).toEqual([]);
    } finally {
      await workspaceV2CoverageReporter.stop(page);
  ...
  test("captures Input Mapping V2 mouse drag from live input and keeps visual capture states observable", async ({ page }) => {
    ...
    try {
      ...
      await page.locator("#inputMappingV2ActionSelect").selectOption("rotateLeft");
      await page.locator("#inputMappingV2AddActionButton").click();
      await armInputMappingCapture(page, "mouse", "MousePrimaryDrag");
      ...
      await page.locator("#inputMappingV2AddActionButton").click();
      await armInputMappingCapture(page, "mouse", "MousePrimaryDragRelease");
      await expect(page.locator("#inputMappingV2CaptureMessage")).toHaveAttribute("data-input-mapping-capture-state", "waiting");
      await page.evaluate(() => {
        window.dispatchEvent(new MouseEvent("mousedown", {
          bubbles: true,
  ...
  test("launches Input Mapping V2 and captures keyboard mappings", async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator("#inputMappingV2GestureList")).not.toContainText("Wheel Left");
      await expect(page.locator("#inputMappingV2GestureList")).not.toContainText("Wheel Right");
      ...
      const gameControllerGestureGroup = page.locator(".input-mapping-v2__gesture-group", { hasText: "Game Controller" });
      await expect(page.locator(".input-mapping-v2__gesture-button[data-input-mapping-gesture-binding='GameControllerButton']")).toHaveCount(0);
      await expect(gameControllerGestureGroup).toContainText("Btn Press");
      await expect(gameControllerGestureGroup).toContainText("Btn Hold");
      await expect(gameControllerGestureGroup).toContainText("Btn Release");
      await expect(gameControllerGestureGroup).toContainText("DPad");
      ...
      await expect(page.locator(".input-mapping-v2__mapping-card")).toHaveCount(0);
      await expect(page.locator("#inputMappingV2CaptureKeyboardButton")).toBeDisabled();
      await expect(page.locator("#inputMappingV2CaptureMouseButton")).toBeDisabled();
      await expect(page.locator("#captureInputContent").locator("xpath=..")).toHaveClass(/input-mapping-v2__section-disabled/);
      await expect(page.locator("#gestureSetupContent").locator("xpath=..")).toHaveClass(/input-mapping-v2__section-disabled/);
      await expect(page.locator(".input-mapping-v2__mapping-card")).toHaveCount(0);
      await expect(page.locator("#inputMappingV2CaptureKeyboardButton")).not.toHaveClass(/is-capturing/);
      await expect(page.locator("#inputMappingV2UsedInputHighlights")).toHaveCount(0);
      ...
      await expect(page.locator(".input-mapping-v2__gamepad-capture-button.is-capturing")).toHaveCount(0);
      ...
      expect(Math.round(emptyMappingTileBox.width)).toBe(250);
      expect(Math.round(emptyMappingTileBox.height)).toBe(225);
      await expect(page.locator(".input-mapping-v2__gesture-button[data-input-mapping-gesture-binding='KeyboardCombo']")).toBeDisabled();
      await expect(page.locator("#previewOutput")).not.toContainText("Combo, Ctrl + R");
      await armInputMappingCapture(page, "keyboard", "KeyboardCombo");
      await page.evaluate(() => {
        window.dispatchEvent(new KeyboardEvent("keydown", {
          bubbles: true,
      ...
      await armInputMappingCapture(page, "mouse", "MouseCombo");
      await page.evaluate(() => {
        window.dispatchEvent(new KeyboardEvent("keydown", {
          bubbles: true,
          cancelable: true,
      ...
      await page.locator("#inputMappingV2CaptureKeyboardButton").click();
      await expect(page.locator("#inputMappingV2CaptureKeyboardButton")).not.toHaveClass(/is-capturing/);
      await expect(page.locator("#statusLog")).toHaveValue(/WARN Keyboard capture canceled for Move Left\./);
      await page.evaluate(() => {
        window.__inputMappingV2CaptureTimeoutMs = 500;
      });
      await armInputMappingCapture(page, "mouse", "MouseClick");
      await expect(page.locator("#inputMappingV2CaptureMouseButton")).toHaveClass(/is-capturing/);
      await expect(page.locator("#inputMappingV2CaptureMouseButton")).not.toHaveClass(/is-capturing/, { timeout: 2000 });
      await expect(page.locator("#statusLog")).toHaveValue(/WARN Mouse capture timed out for Move Left\./);
      await page.evaluate(() => {
        delete window.__inputMappingV2CaptureTimeoutMs;
      });
      await armInputMappingCapture(page, "keyboard", "KeyboardPress");
      ...
      await expect(page.locator(".input-mapping-v2__mapping-card.is-selected")).toContainText("Move Left");
      await page.locator(".input-mapping-v2__mapping-card[data-input-mapping-tile-action-id='confirm']").click({ position: { x: 12, y: 12 } });
      ...
      await expect(page.locator(".input-mapping-v2__gesture-button[data-input-mapping-gesture-binding='GameControllerCombo']")).toBeDisabled();
      await expect(page.locator("#previewOutput")).not.toContainText("Combo, GD Button 1 + Alt");
      await armInputMappingCapture(page, "gamepad", "GameControllerCombo");
      await expect(page.locator("#inputMappingV2CaptureMessage")).toContainText("Combo capture recorded Game Controller Button 1");
      await page.evaluate(() => {
        window.dispatchEvent(new KeyboardEvent("keydown", {
      ...
      await expect(page.locator("#previewOutput")).toContainText("Combo, GD Button 1 + Alt");
      await expect(page.locator("#inspectorOutput")).toContainText('"binding": "Combo:Pad0:Button1+AltLeft"');
      await page.evaluate(() => {
        window.__inputMappingV2MockGamepads[0] = {
          ...window.__inputMappingV2MockGamepads[0],
      ...
      await armInputMappingCapture(page, "gamepad", "GameControllerButtonPress", { gamepadIndex: 1 });
      ...
      await expect(page.locator("#previewOutput")).toContainText("Keyboard, KeyD, Press");
      await expect(page.locator("#previewOutput")).toContainText("GD, X, Btn Press");
      ...
      await expect(page.locator("#inputMappingV2UsedInputHighlights")).toHaveCount(0);
      await expect(page.locator(".input-mapping-v2__input-token", { hasText: "GD" }).filter({ hasText: "X" })).toHaveAttribute("title", "Logitech RumblePad 2 USB\nSTANDARD GAMEPAD\nVendor: 046d Product: c218\nX\nBtn Press");
      await expect(page.locator("#inspectorOutput")).toContainText('"source": "gamepad"');
      await expect(page.locator("#inspectorOutput")).toContainText('"binding": "Pad1:Button2:GameControllerButtonPress"');
      await expect(page.locator("#inspectorOutput")).toContainText('"label": "Game Controller X Btn Press"');
      await page.locator(".input-mapping-v2__input-token", { hasText: "GD" }).filter({ hasText: "X" }).click();
      await expect(page.locator(".input-mapping-v2__input-token", { hasText: "GD" }).filter({ hasText: "X" })).toHaveCount(0);
      await expect(page.locator("#statusLog")).toHaveValue(/OK Deleted Game Controller X Btn Press from Move Left\./);
      await page.evaluate(() => {
        window.__inputMappingV2MockGamepads[0] = {
          ...window.__inputMappingV2MockGamepads[0],
          buttons: [
      ...
      await armInputMappingCapture(page, "gamepad", "GameControllerButtonPress");
      await expect(page.locator(".input-mapping-v2__gamepad-capture-button[data-input-mapping-gamepad-index='0']")).toHaveClass(/is-capturing/);
      await page.evaluate(() => {
        window.__inputMappingV2MockGamepads[0] = {
          ...window.__inputMappingV2MockGamepads[0],
          buttons: [
      ...
      await expect(page.locator("#previewOutput")).toContainText("GD, Button 5, Btn Press");
      await expect(page.locator("#inspectorOutput")).toContainText('"binding": "Pad0:Button5:GameControllerButtonPress"');
      const button5Token = page.locator(".input-mapping-v2__input-token[data-input-mapping-binding='Pad0:Button5:GameControllerButtonPress']");
      await page.evaluate(() => {
        window.__inputMappingV2MockGamepads[0] = {
          ...window.__inputMappingV2MockGamepads[0],
          buttons: [
      ...
      await page.locator(".input-mapping-v2__input-token", { hasText: "GD" }).filter({ hasText: "Button 5" }).click();
      await expect(page.locator(".input-mapping-v2__input-token", { hasText: "GD" }).filter({ hasText: "Button 5" })).toHaveCount(0);
      await expect(page.locator(".input-mapping-v2__input-token[data-input-mapping-binding='KeyD']")).toHaveCount(1);
      await expect(page.locator("#statusLog")).toHaveValue(/OK Deleted Game Controller Button 5 Btn Press from Move Left\./);
      ...
      expect(asteroidsManifestInputMapping.actions.map((action) => action.label)).toEqual([
        "Start One Player",
        "Start Two Player",
        "Continue",
        "Rotate Left",
      ...
      expect(asteroidsManifestInputMapping.actions.every((action) => action.inputs.length > 0)).toBe(true);
  ...
  test("launches Storage Inspector V2 with V2 labels, panels, theme, and delete controls", async ({ page }) => {
    const pageErrors = [];
    await page.setViewportSize({ height: 900, width: 1440 });
    await page.addInitScript(() => {
  ...
  test("uses header lifecycle controls and launches tools from fixed Workspace Manager V2 tiles", async ({ page }) => {
    ...
    try {
      ...
      expect(toolGroupMembership).toEqual({
        ...
        Utilities: ["Input Mapping V2", "Preview Generator V2", "Text to Speech V2", "Audio / SFX Playground V2", "MIDI Studio V2"],
      ...
      expect(selectedGameHydration.schemaByTool["audio-sfx-playground-v2"]).toMatchObject({
        source: "workspace-manager-v2",
      ...
      expectRuntimeBindingMetadata(selectedGameHydration.workspaceByTool["audio-sfx-playground-v2"]);
      ...
      expectRuntimeBindingMetadata(selectedGameHydration.workspaceByTool["midi-studio-v2"]);
      ...
      expectRuntimeBindingMetadata(selectedGameHydration.workspaceByTool["world-vector-studio-v2"]);
      ...
      expect(Object.values(selectedGameHydration.dirtyByTool)).toHaveLength(11);
      ...
      const midiStudioTile = page.locator('[data-workspace-tool-id="midi-studio-v2"]');
      ...
      const audioSfxToolTile = page.locator('[data-workspace-tool-id="audio-sfx-playground-v2"]');
      ...
      await expect(midiStudioTile).toBeEnabled();
      await expect(midiStudioTile).toContainText("MIDI Studio V2");
      await expect(midiStudioTile).toContainText("Ready to launch");
      await expect(midiStudioTile).toContainText("Schema-valid manifest");
      ...
      await expect(audioSfxToolTile).toBeEnabled();
      await expect(audioSfxToolTile).toContainText("Audio / SFX Playground V2");
      await expect(audioSfxToolTile).toContainText("Ready to launch");
      await expect(audioSfxToolTile).toContainText("Browser-generated SFX payloads");
      ...
      await expect(page.locator("#statusLog")).toHaveValue(/OK Hydrated workspace session for asset-manager-v2, palette-manager-v2, object-vector-studio-v2, world-vector-studio-v2, collision-inspector-v2, input-mapping-v2, preview-generator-v2, text2speech-V2, audio-sfx-playground-v2, midi-studio-v2, storage-inspector-v2\./);
      ...
      await expect(page.locator("#storageInspectorV2EntryList [data-storage-inspector-v2-entry-id='sessionStorage:workspace.tools.audio-sfx-playground-v2']")).toHaveCount(1);
      ...
      await expect(page.locator("#storageInspectorV2EntryList [data-storage-inspector-v2-entry-id='sessionStorage:workspace.tools.midi-studio-v2']")).toHaveCount(1);
      ...
      await expect(page.locator("#log")).toContainText("OK Workspace repo session reference loaded from workspace.repo.reference for HTML-JavaScript-Gaming.", { timeout: 20000 });
      ...
      await expect(page.locator(".input-mapping-v2__mapping-card")).toHaveCount(13);
      await expect(page.locator(".input-mapping-v2__mapping-card[data-input-mapping-tile-action-id='rotateLeft']")).toContainText("Rotate Left");
      await expect(page.locator(".input-mapping-v2__mapping-card[data-input-mapping-tile-action-id='rotateLeft']")).toContainText("Keyboard, ArrowLeft, Hold");
      await expect(page.locator("#statusLog")).toHaveValue(/OK Loaded 13 source-derived action mappings from Workspace Manager launch data\./);
      await page.locator("#returnToWorkspaceButton").click();
      await expect(page).toHaveURL(/workspace-manager-v2\/index\.html\?.*hostContextId=workspace-manager-v2-/);
      await expectWorkspaceReturnedFromTool(page, { dirty: true });
      expect(pageErrors).toEqual([]);
  ...
  test("saves empty Text to Speech V2 arrays through workspace return and manifest write-back", async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator("#statusLog")).toHaveValue(/INFO Saved toolState items: 5 \(asset-manager-v2 assets=15; input-mapping-v2 actions=13 inputs=13; object-vector-studio-v2 objects=7; palette-manager-v2 swatches=10; text2speech-V2 queue=0\)\./);
  ...
  test("syncs Workspace Manager V2 dirty lifecycle buttons and closes clean toolState data", async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator("#statusLog")).toHaveValue(/INFO Saved toolState items: (?:4 \(asset-manager-v2 assets=15; input-mapping-v2 actions=13 inputs=13; object-vector-studio-v2 objects=7; palette-manager-v2 swatches=11\)|5 \(asset-manager-v2 assets=15; input-mapping-v2 actions=13 inputs=13; object-vector-studio-v2 objects=7; palette-manager-v2 swatches=11; text2speech-V2 queue=(?:0|1)\))\./);
      await expect(page.locator("#statusLog")).toHaveValue(/OK Save validation result: game manifest valid; root\.tools toolState valid; saved context matched re-read file\./);
      await expect(page.locator("#statusLog")).toHaveValue(/OK Save dirty\/clean validation: 1 dirty toolState payload persisted; 1 toolState key marked clean\./);
  ...
  test("owns temporary UAT manifest seeding and launches Asset Manager V2 through session context", async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator('[data-workspace-tool-id="audio-sfx-playground-v2"]')).toContainText("Audio / SFX Playground V2");
      await expect(page.locator('[data-workspace-tool-id="audio-sfx-playground-v2"]')).toContainText("Ready to launch");
      ...
      await page.locator('[data-workspace-tool-id="audio-sfx-playground-v2"]').click();
      await expect(page).toHaveURL(/audio-sfx-playground-v2\/index\.html.*launch=workspace/);
      await expect(page.locator("body[data-tool-id='audio-sfx-playground-v2']")).toBeVisible();
      await expect(page.locator(".tools-platform-frame__title[data-tool-id='audio-sfx-playground-v2']")).toHaveText("Audio / SFX Playground V2");
      await expect(page.locator(".audio-sfx__workspace__menu")).toBeVisible();
      await expect(page.locator(".audio-sfx__tool__menu")).toBeHidden();
      await expect(page.locator("#statusLog")).toHaveValue(/Audio \/ SFX Playground V2 ready\./);
      await page.locator("#returnToWorkspaceButton").click();
      await expect(page).toHaveURL(/workspace-manager-v2\/index\.html.*hostContextId=workspace-manager-v2-/);
      await expect(page).toHaveURL(/workspace=uat/);
      await expect(page.locator('[data-workspace-tool-id="asset-manager-v2"]')).toBeDisabled();
