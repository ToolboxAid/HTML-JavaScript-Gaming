...
test("Idea Board uses accordion table ideas and notes", async ({ page }) => {
  ...
  try {
    ...
    await page.getByRole("link", { name: "Open Journey" }).click();
    ...
    await page.locator("[data-journey-note-button]", { hasText: "Source Idea: Lantern Reef" }).click();
    const generatedJourneyItem = page.locator("[data-journey-item-button]", { hasText: "Use dusk tide changes as the first Game Hub planning note." }).first();
    await expect(generatedJourneyItem).toBeVisible();
    await generatedJourneyItem.click();
    ...
    await page.locator("[data-journey-title-input]").fill("Refine dusk tide puzzle loop");
    await page.locator("[data-journey-item-details-input]").fill("Keep this as an editable journey planning item.");
    await page.getByLabel("Status").selectOption("in-progress");
    await page.getByRole("button", { name: "Update Item" }).click();
    await expect(page.locator("[data-journey-item-tree]")).toContainText("Refine dusk tide puzzle loop");
    await expect(page.locator("[data-journey-item-tree]")).toContainText("Keep this as an editable journey planning item.");
    await page.goto(createdProjectUrl, { waitUntil: "networkidle" });
    await expect(page.locator("[data-source-idea-notes]")).toContainText("Use dusk tide changes as the first Game Hub planning note.");
    await expect(page.locator("[data-source-idea-notes]")).not.toContainText("Refine dusk tide puzzle loop");
