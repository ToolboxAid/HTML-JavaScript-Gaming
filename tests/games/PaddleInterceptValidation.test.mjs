...
function createPad(index, { pressed = {}, connected = true } = {}) {
  return {
    index,
    connected,
    id: `Pad-${index}`,
...
export async function run() {
  ...
  const noDocument = bootPaddleIntercept({
    documentRef: null,
    EngineClass: class {
      constructor() {
        constructed = true;
  ...
  const noCanvas = bootPaddleIntercept({
    documentRef: {
      getElementById() {
        return null;
      },
  ...
  const booted = bootPaddleIntercept({
    documentRef: {
      getElementById(id) {
  ...
  assert.equal(installedScene instanceof PaddleInterceptScene, true);
  ...
  const scene = new PaddleInterceptScene();
  ...
  const pausedX = scene.world.ball.centerX;
  input.setState([]);
  scene.update(0.3);
  assert.equal(scene.world.ball.centerX, pausedX);
