...
David Quesenberry
06/02/2026
Wave2ToolStateBoundaryValidation.test.mjs
*/
...
import {
  WAVE_2_TOOL_IDS,
  getWave2ToolContract,
  readScenarios,
} from "./Wave2ToolContractBaselineValidation.test.mjs";

const WAVE_2_TOOLSTATE_ERRORS = Object.freeze({
  ACTIVE_OWNERSHIP_INVALID: "WAVE2_TOOLSTATE_ACTIVE_OWNERSHIP_INVALID",
  PAYLOAD_INVALID: "WAVE2_TOOLSTATE_PAYLOAD_INVALID",
  PAYLOAD_FIELD_FORBIDDEN: "WAVE2_TOOLSTATE_PAYLOAD_FIELD_FORBIDDEN",
});
...
export function run() {
  ...
  for (const scenario of scenarios.validToolStates) {
    ...
    const validation = validateWave2ToolStateBoundary(scenario.toolState, {
...
export function validateWave2ToolStateBoundary(toolState, {
  toolId,
  requiredPayloadField,
} = {}) {
  const errors = [];
  const toolContract = getWave2ToolContract(toolId);
  ...
  if (!toolStateLinksToToolContract(toolState, toolContract)) {
    errors.push(error(WAVE_2_TOOLSTATE_ERRORS.ACTIVE_OWNERSHIP_INVALID, "toolType"));
  }
  ...
  if (!toolState?.payload || typeof toolState.payload !== "object" || Array.isArray(toolState.payload) || !Object.hasOwn(toolState.payload, requiredPayloadField)) {
    errors.push(error(WAVE_2_TOOLSTATE_ERRORS.PAYLOAD_INVALID, "payload"));
  } else {
    for (const fieldName of FORBIDDEN_PAYLOAD_FIELDS) {
      if (Object.hasOwn(toolState.payload, fieldName)) {
        errors.push(error(WAVE_2_TOOLSTATE_ERRORS.PAYLOAD_FIELD_FORBIDDEN, `payload.${fieldName}`));
      }
    }
  }
