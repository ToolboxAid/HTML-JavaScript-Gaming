...
export function testCanvasText(assert) {
    ...
    const singleLine = CanvasText._renderTextToContext(mockCtx, 'hello', 10, 20, {
        fontSize: 16,
        fontFamily: 'monospace',
        color: 'red',
        useDpr: false
    });
    assert(singleLine.x === 10 && singleLine.y === 20, 'CanvasText._renderTextToContext should report rendered coordinates');
    assert(mockCtx.saveCalls === 1 && mockCtx.restoreCalls === 1, 'CanvasText._renderTextToContext should preserve canvas state');
    assert(mockCtx.fillTextCalls.length === 1, 'CanvasText._renderTextToContext should draw one line of text');

    CanvasText._renderTextToContext(mockCtx, 'font-override', 12, 24, {
        font: 'bold 18px Segoe UI',
        useDpr: false
    });
    assert(mockCtx.font === 'bold 18px Segoe UI', 'CanvasText._renderTextToContext should allow a raw font override');

    const multiLine = CanvasText._renderMultilineTextToContext(mockCtx, ['line1', 'line2'], 15, 25, {
        fontSize: 12,
        lineHeight: 20,
        useDpr: false
    });
    assert(multiLine.length === 2, 'CanvasText._renderMultilineTextToContext should return one result per line');
    assert(mockCtx.fillTextCalls.length === 4, 'CanvasText._renderMultilineTextToContext should draw all provided lines');

    const centered = CanvasText._renderCenteredTextToContext(mockCtx, 'centered', 50, {
        centerX: 100,
        fontSize: 20,
        useDpr: false
    });
    ...
    assert(centeredLines.length === 2, 'CanvasText._renderCenteredMultilineTextToContext should draw one result per centered line');
