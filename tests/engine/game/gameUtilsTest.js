...
export function testGameUtils(assert) {
    ...
    const swap1 = GameTurnFlowUtils.swapPlayer(playerLives, currentPlayer, playerCount);
    ...
    currentPlayer = 1;
    const swap2 = GameTurnFlowUtils.swapPlayer(playerLives, currentPlayer, playerCount);
    ...
    playerLives = [0, 2, 0, 3];
    currentPlayer = 3;
    const swap3 = GameTurnFlowUtils.swapPlayer(playerLives, currentPlayer, playerCount);
    ...
    const swapGameOver = GameTurnFlowUtils.swapPlayer(playerLives, currentPlayer, playerCount);
    ...
    const swapTrackedGameOver = GameTurnFlowUtils.swapPlayer([1, 0, 5], 0, 2);
    ...
    for (const testCase of equivalenceCases) {
        const gameUtilsResult = GameTurnFlowUtils.swapPlayer(testCase.lives, testCase.current, testCase.count);
        const turnFlowResult = GameTurnFlowUtils.swapPlayer(testCase.lives, testCase.current, testCase.count);

        assert(JSON.stringify(gameUtilsResult) === JSON.stringify(turnFlowResult), "swapPlayer equivalence failed");
        assert(JSON.stringify(testCase.lives) === JSON.stringify(testCase.lives), "input mutation detected");
        assert(
            GameTurnFlowUtils.areTrackedPlayersOut(testCase.lives, testCase.count) === GameTurnFlowUtils.areTrackedPlayersOut(testCase.lives, testCase.count),
            "areTrackedPlayersOut equivalence failed"
        );
        assert(
            GameTurnFlowUtils.findNextActivePlayer(testCase.lives, testCase.current, testCase.count) === GameTurnFlowUtils.findNextActivePlayer(testCase.lives, testCase.current, testCase.count),
            "findNextActivePlayer equivalence failed"
        );
    }
}
