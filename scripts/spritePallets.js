...
class SpritePallets {
    static default = [
        ...
        { symbol: 'Ù', hex: '#FFB6C1', name: 'LightPink' },
    ...
    // Dump the pallet to the console
    static dumpPallet(paletteName = 'default') {
        try {
            ...
            console.error(`Failed to retrieve details for palette "${paletteName}":`, error);
        }
    }

    // Generic method to get palette by name
    static getPalette(paletteName) {
        if (SpritePallets[paletteName]) {
            return SpritePallets[paletteName];
        } else {
    ...
    // Get data by index in a specific palette
    static getByIndex(paletteName, index) {
        const palette = SpritePallets.getPalette(paletteName);
        if (palette && index >= 0 && index < palette.length) {
            return palette[index];
        } else {
            console.error("Index out of bounds or palette not found");
            return null;
        }
    }

    // Get data by symbol in a specific palette
    static getBySymbol(paletteName, symbol) {
        const palette = SpritePallets.getPalette(paletteName);
        if (palette) {
            const result = palette.find(pallet => pallet.symbol === symbol);
            if (result) {
                return result;
            } else {
                console.error(`Symbol '${symbol}' not found in palette '${paletteName}'`);
                return null;
            }
        }
        return null;
    ...
    // Get data by hex in a specific palette
    static getByHex(paletteName, hex) {
        const palette = SpritePallets.getPalette(paletteName);
        if (palette) {
            const result = palette.find(pallet => pallet.hex.toLowerCase() === hex.toLowerCase());
            if (result) {
                return result;
            } else {
                console.error(`Hex '${hex}' not found in palette '${paletteName}'`);
                return null;
            }
        }
        return null;
