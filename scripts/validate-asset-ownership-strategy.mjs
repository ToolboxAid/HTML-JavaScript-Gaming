...
const TOOL_DEMO_PROJECT_ASSETS_PATH = "toolbox/shared/samples/project-asset-registry-demo/project.assets.json";
...
function ownerPrefixForPath(repoPath) {
  ...
  if (segments[0] === "tools" && segments.length >= 2) {
    ...
    if (segments[1] === "shared" && segments.length >= 5 && segments[2] === "samples") {
      return `toolbox/shared/samples/${segments[3]}`;
    }
...
export async function validateAssetOwnershipStrategy({ emitLogs = true } = {}) {
  ...
  const toolDemoOwnerPrefix = "toolbox/shared/samples/project-asset-registry-demo";
