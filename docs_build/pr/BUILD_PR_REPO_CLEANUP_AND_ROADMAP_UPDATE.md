...
1. updates `docs_build/dev/roadmaps/MASTER_ROADMAP_ENGINE.md` conservatively using bracket-only state changes
...
- In `docs_build/dev/roadmaps/MASTER_ROADMAP_ENGINE.md`, only bracket-state updates are allowed: `[ ]`, `[.]`, `[x]`.
...
Apply only conservative bracket-state updates in `docs_build/dev/roadmaps/MASTER_ROADMAP_ENGINE.md` for the already-planned status deltas below.
...
Create `docs_build/reports/cleanup_live_reference_inventory.txt` with a precise inventory for each cleanup target.
...
### 4) BUILD report
Create `docs_build/reports/BUILD_PR_REPO_CLEANUP_AND_ROADMAP_UPDATE_report.md` with:
