...
- Keep scope limited to `tools/Sprite Editor/`, `tools/index.html` only if needed, and docs_build/reports.
...
- No changes outside `tools/Sprite Editor/` + docs_build/reports (+ `tools/index.html` only if strictly needed).
