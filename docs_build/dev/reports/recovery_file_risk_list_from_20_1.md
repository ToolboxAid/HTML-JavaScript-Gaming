...
- `docs_build/dev/start_of_day/codex/PROJECT_INSTRUCTIONS.lnk` (added)
  - Risk: start_of_day boundary touched.
- `docs_build/dev/roadmaps/MASTER_ROADMAP_SAMPLES2TOOLS.md` (added)
...
## Medium Risk Files
- `docs_build/dev/reports/*` (large batch additions)
  - Risk: not runtime-critical but adds noise during recovery triage.
- `docs_build/pr/*` (multiple new PR/PLAN docs post-baseline)
