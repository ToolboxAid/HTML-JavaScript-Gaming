...
Commands run:
- `rg -n "Session Library|Workspace Session|savedSessions|activeSession|session payload|saved session|active session|Create Session \+ Launch|Load Fixture|New Session" docs_build/dev/reports --glob "PR_*"`
...
Result counts:
- `docs_build/dev/reports`: `123` matches
