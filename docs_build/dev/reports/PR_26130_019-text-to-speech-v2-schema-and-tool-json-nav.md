...
- Removed planned roadmap cards from `toolbox/index.html` for Browser Speech Backend, eSpeak NG WASM Backend, Queue-Based Speech Playback, Offline / Local Speech Support, and Raspberry Pi Speech Deployment.
- Kept unrelated tool schemas unchanged; only `toolbox/schemas/tools/text2speach-V2.schema.json` was updated.
...
- Full samples smoke test skipped: this PR is limited to Text to Speech V2 schema/tool JSON navigation and `toolbox/index.html` planned-section cleanup; it does not modify broad sample launch/runtime paths.
