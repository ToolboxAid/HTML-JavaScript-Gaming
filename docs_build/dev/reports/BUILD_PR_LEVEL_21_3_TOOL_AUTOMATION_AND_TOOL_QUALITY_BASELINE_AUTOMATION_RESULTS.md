...
| `node ./tests/runtime/LaunchSmokeAllEntries.test.mjs --tools` | PASS | `PASS=17 FAIL=0 TOTAL=17` and `docs_build/dev/reports/launch_smoke_report.md` updated |
...
| `node ./scripts/validate-active-tools-surface.mjs` | FAIL | Fails with `ENOENT` for missing `docs_build/pr/BUILD_PR_VECTOR_SHOWCASE_AND_GEOMETRY_RUNTIME_FINAL.md` dependency (captured in `tool_known_bugs.md`, TB-002) |
