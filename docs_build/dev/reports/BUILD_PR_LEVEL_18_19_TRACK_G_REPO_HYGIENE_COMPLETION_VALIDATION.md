...
1. Roadmap target check:
   - rg -n "Track G|Remove imports to export|remove unnecessary|remove empty folders|validate folder ownership rules|enforce clean repo structure" docs_build/dev/roadmaps/MASTER_ROADMAP_ENGINE.md
2. `.keep` inventory:
   - rg --files -g "**/.keep"
...
5. start_of_day safety check:
   - git status --short -- docs_build/dev/start_of_day
