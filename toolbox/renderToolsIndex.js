...
const SAMPLES_INDEX_PATH = "/archive/v1-v2/samples/index.html";
const SAMPLES_METADATA_PATH = "/archive/v1-v2/samples/metadata/samples.index.metadata.json";

function toStandaloneHref(entryPoint) {
  const normalized = String(entryPoint || "").replace(/^\.?\/*/, "");
  return normalized ? `/toolbox/${normalized}` : "#";
}
