...
One good current example is `samples/Solar System/`, which now runs as an engine-driven sample using `GameBase`, engine-owned keyboard input, and `GameObjectSystem` ownership for its active celestial bodies.
...
`samples/Solar System/` is a concrete example of this direction: the sample uses `GameObjectSystem` to own active simulation objects while keeping the game shell focused on input, state, update, and render flow.
