# 3D Camera Path Editor Testing Guide
...
## Purpose Of Testing
3D camera waypoint/timeline authoring and JSON export workflows.
...
## Setup Steps
1. Open `tools/index.html` and launch `3D Camera Path Editor`.
...
## Smoke Tests
1. Launch the tool directly via `tools/3D Camera Path Editor/index.html` and verify first paint in under expected local baseline.
...
4. Integration handoff: Exported camera path payload is consumable by runtime camera-path consumers.
