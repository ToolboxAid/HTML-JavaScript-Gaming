# Vector Map Editor Testing Guide

## Tool
- id: `vector-map-editor`
- entry point: `tools/Vector Map Editor/index.html`

## Purpose Of Testing
Vector map layout and runtime geometry handoff workflows.
...
## Setup Steps
1. Open `tools/index.html` and launch `Vector Map Editor`.
...
## Smoke Tests
1. Launch the tool directly via `tools/Vector Map Editor/index.html` and verify first paint in under expected local baseline.
...
4. Integration handoff: Exported map payload is consumable by sample/runtime map loaders (for example sample 1208 lanes).
