...
## Purpose Of Testing
3D metadata inspection and asset verification workflows.
...
## Setup Steps
1. Open `tools/index.html` and launch `3D Asset Viewer`.
...
## Smoke Tests
1. Launch the tool directly via `tools/3D Asset Viewer/index.html` and verify first paint in under expected local baseline.
...
4. Integration handoff: Viewer metadata aligns with 3D asset payload consumed by runtime systems.
