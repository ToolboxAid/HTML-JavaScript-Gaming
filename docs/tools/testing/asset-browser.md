# Asset Browser / Import Hub Testing Guide
...
## Purpose Of Testing
Asset browsing, ingestion planning, and import-hub workflows.
...
## Setup Steps
1. Open `tools/index.html` and launch `Asset Browser / Import Hub`.
...
## Smoke Tests
1. Launch the tool directly via `tools/Asset Browser/index.html` and verify first paint in under expected local baseline.
...
3. Save/export workflow: Verify save/export controls are clearly not applicable (or disabled) and tool remains stable without export support.
4. Integration handoff: Import planning output is accepted by downstream asset pipeline tooling.
