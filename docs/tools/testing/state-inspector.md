...
## Purpose Of Testing
Read-only runtime state inspection and payload visibility workflows.
...
## Setup Steps
1. Open `tools/index.html` and launch `State Inspector`.
...
## Smoke Tests
1. Launch the tool directly via `tools/State Inspector/index.html` and verify first paint in under expected local baseline.
...
4. Integration handoff: State payload inspection aligns with Tool Host/runtime state provider output.
