...
## Purpose Of Testing
3D map layout and normalized JSON export workflows.
...
## Setup Steps
1. Open `tools/index.html` and launch `3D Map Editor`.
...
## Smoke Tests
1. Launch the tool directly via `tools/3D Map Editor/index.html` and verify first paint in under expected local baseline.
...
4. Integration handoff: Exported 3D map payload is consumable by 3D sample/runtime map loaders.
