PR-005 — engine/game factual export capture build

### Purpose

This BUILD_PR creates the docs-only structure for recording the actual current
`engine/game` export surface from the repo.

### Scope

This patch records the factual capture framework only.
It does not change runtime behavior, imports, file locations, or execution paths.

### What This BUILD_PR Does

- adds docs for factual capture of actual `engine/game` exports
- records the expected capture fields
