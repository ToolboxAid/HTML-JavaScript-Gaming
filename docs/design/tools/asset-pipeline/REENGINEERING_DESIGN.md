# Asset Pipeline Reengineering Design (asset-pipeline)

## Purpose
- Launchable shared pipeline surface for load, validate, normalize, and emit contract checks.

## Current V1 Capability
- Active in registry-driven tools surface.
- Runtime entry point: `Asset Pipeline/index.html`.
- Runtime implementation file: `tools/Asset Pipeline/main.js`.
...
- Schema ref: `tools/schemas/tools/asset-pipeline.schema.json`. Required root keys: `pipelinePayload`. Defined root properties: `pipelinePayload`.
...
- Top-level functions: `setOutput()`, `normalizeText()`, `cloneJson()`, `toSlug()`, `readLaunchContextFromQuery()`, `normalizeSamplePresetPath()`, `buildPresetLoadedStatus()`, `buildPresetLoadedWithContextStatus()`, `applyLaunchContextToPayload()`, `setStatus()`, `setInputValue()`, `parseJsonObjectString()`, `createEmptyDomainInputs()`, `normalizeAssetId()`, `deriveGameIdFromManifest()`, `appendDomainRecord()`, `appendDomainRecordsFromToolIntegration()`, `normalizeCatalogPath()`, `normalizeExplicitCatalogPath()`, `buildCatalogPathCandidates()`, `inferDomainFromCatalogKind()`, `inferSourceToolIdForDomain()`, `appendDomainRecordsFromCatalogEntries()`, `extractPipelinePayloadFromSource()`, `readActiveProjectManifest()`, `getInputPayload()`, `runPipeline()`, `loadPipelinePayloadIntoInput()`, `bootAssetPipelineTool()`.
...
- Import: `readLaunchContextFromQuery()`, `buildPresetLoadedStatus()`, `buildPresetLoadedWithContextStatus()`, `applyLaunchContextToPayload()`, `parseJsonObjectString()`, `extractPipelinePayloadFromSource()`, `readActiveProjectManifest()`, `getInputPayload()`, `loadPipelinePayloadIntoInput()`
- Validate: `normalizeText()`, `normalizeSamplePresetPath()`, `normalizeAssetId()`, `normalizeCatalogPath()`, `normalizeExplicitCatalogPath()`
- Edit/process: `applyLaunchContextToPayload()`
...
- Publish to `tools.asset-pipeline`: Supported as target ownership in design; concrete publish path varies by tool.
