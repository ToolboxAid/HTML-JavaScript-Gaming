# State Inspector Reengineering Design (state-inspector)

## Purpose
- Read-only state visibility tool for host context, storage snapshots, and structured runtime payload inspection.

## Current V1 Capability
- Active in registry-driven tools surface.
- Runtime entry point: `State Inspector/index.html`.
- Runtime implementation file: `tools/State Inspector/main.js`.
...
- Schema ref: `tools/schemas/tools/state-inspector.schema.json`. Required root keys: `snapshot`. Defined root properties: `snapshot`.
...
- Top-level functions: `normalizeSamplePresetPath()`, `buildPresetLoadedStatus()`, `setStatus()`, `isEmptySnapshotPayload()`, `emitManualJsonDiagnostic()`, `isManualJsonInputEmpty()`, `updateInspectJsonActionState()`, `clearRoutedPayloadQueryParam()`, `readRoutedInspectionPayload()`, `readStorageEntries()`, `readProjectManifest()`, `readBootRegistryKeys()`, `buildLiveSnapshot()`, `renderSnapshot()`, `refreshSnapshot()`, `inspectInputJson()`, `bindEvents()`, `bootStateInspector()`.
...
- Import: `buildPresetLoadedStatus()`, `isEmptySnapshotPayload()`, `clearRoutedPayloadQueryParam()`, `readRoutedInspectionPayload()`, `readStorageEntries()`, `readProjectManifest()`, `readBootRegistryKeys()`
- Validate: `normalizeSamplePresetPath()`
- Edit/process: `updateInspectJsonActionState()`, `renderSnapshot()`
...
- Publish to `tools.state-inspector`: Supported as target ownership in design; concrete publish path varies by tool.
