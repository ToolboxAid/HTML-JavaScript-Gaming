# Replay Visualizer Reengineering Design (replay-visualizer)

## Purpose
- Event replay timeline viewer for stepping, autoplay, and frame inspection of runtime event streams.

## Current V1 Capability
- Active in registry-driven tools surface.
- Runtime entry point: `Replay Visualizer/index.html`.
- Runtime implementation file: `tools/Replay Visualizer/main.js`.
...
- Schema ref: `tools/schemas/tools/replay-visualizer.schema.json`. Required root keys: `events`. Defined root properties: `events`.
...
- Top-level functions: `normalizeSamplePresetPath()`, `buildPresetLoadedStatus()`, `setStatus()`, `getDurationMs()`, `clampTimeMs()`, `setCurrentTimeMs()`, `renderEventList()`, `renderCurrentEvent()`, `applyEvents()`, `loadReplayFromInput()`, `stopPlayback()`, `updateControlState()`, `playReplay()`, `bindEvents()`, `bootReplayVisualizer()`.
...
- Import: `buildPresetLoadedStatus()`, `loadReplayFromInput()`
- Validate: `normalizeSamplePresetPath()`
- Edit/process: `renderEventList()`, `renderCurrentEvent()`, `applyEvents()`, `updateControlState()`
...
- Publish to `tools.replay-visualizer`: Supported as target ownership in design; concrete publish path varies by tool.
