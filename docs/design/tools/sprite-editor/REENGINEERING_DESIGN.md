# Sprite Editor Reengineering Design (sprite-editor)

## Purpose
- Pixel-art authoring workspace for palette-locked sprite sheets, animation frames, and registry-aware sprite projects.

## Current V1 Capability
- Active in registry-driven tools surface.
- Runtime entry point: `Sprite Editor/index.html`.
- Runtime implementation file: `tools/Sprite Editor/main.js`.
...
- Schema ref: `tools/schemas/tools/sprite-editor.schema.json`. Required root keys: `spriteProject`. Defined root properties: `assetRegistry`, `spriteProject`.
...
- Top-level functions: `bootSpriteEditor()`, `setupFullscreenSidePanels()`.
...
- Import: No explicit import helper surfaced in current file inventory.
- Validate: No explicit validate helper surfaced in current file inventory.
- Edit/process: `bootSpriteEditor()`
...
- Publish to `tools.sprite-editor`: Supported as target ownership in design; concrete publish path varies by tool.
