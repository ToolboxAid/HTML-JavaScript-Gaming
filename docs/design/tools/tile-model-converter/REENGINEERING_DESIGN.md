# Tile Model Converter Reengineering Design (tile-model-converter)

## Purpose
- Shared asset-converter surface for tile/vector conversion requests using normalized converter contracts.

## Current V1 Capability
- Active in registry-driven tools surface.
- Runtime entry point: `Tile Model Converter/index.html`.
- Runtime implementation file: `tools/Tile Model Converter/main.js`.
...
- Schema ref: `tools/schemas/tools/tile-model-converter.schema.json`. Required root keys: `candidate`, `conversion`. Defined root properties: `candidate`, `conversion`.
...
- Top-level functions: `normalizeSamplePresetPath()`, `buildPresetLoadedStatus()`, `setStatus()`, `getInputPayload()`, `runConversion()`, `bootTileModelConverter()`.
...
- Import: `buildPresetLoadedStatus()`, `getInputPayload()`
- Validate: `normalizeSamplePresetPath()`
- Edit/process: `bootTileModelConverter()`
...
- Publish to `tools.tile-model-converter`: Supported as target ownership in design; concrete publish path varies by tool.
