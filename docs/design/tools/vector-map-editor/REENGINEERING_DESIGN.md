# Vector Map Editor Reengineering Design (vector-map-editor)
...
- Runtime entry point: `Vector Map Editor/index.html`.
- Runtime implementation file: `tools/Vector Map Editor/main.js`.
...
- Schema ref: `tools/schemas/tools/vector-map-editor.schema.json`. Required root keys: `vectorMapDocument`. Defined root properties: `vectorMapDocument`.
...
- Top-level functions: `readHostedVectorMapDocument()`, `applyHostedWorkspacePayload()`, `startVectorMapEditor()`, `bootVectorMapEditor()`.
...
- Import: `readHostedVectorMapDocument()`, `applyHostedWorkspacePayload()`
- Validate: No explicit validate helper surfaced in current file inventory.
- Edit/process: `applyHostedWorkspacePayload()`, `startVectorMapEditor()`, `bootVectorMapEditor()`
...
- Publish to `tools.vector-map-editor`: Supported as target ownership in design; concrete publish path varies by tool.
