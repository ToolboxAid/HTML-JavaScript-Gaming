...
Tool ID: `3d-json-payload`
Source folder: `tools/3D JSON Payload`

## 1. Tool Purpose
Normalize generic 3D map payload JSON and present canonical output for downstream tool or runtime use.
...
- `tools/3D JSON Payload/main.js`: button #normalize3dMapButton - inferred from JS DOM lookup
- `tools/3D JSON Payload/main.js`: button #openHowToUse3dMapButton - inferred from JS DOM lookup
- `tools/3D JSON Payload/main.js`: panel #map3dStatus - inferred from JS DOM lookup
- `tools/3D JSON Payload/main.js`: input #map3dInput - inferred from JS DOM lookup
- `tools/3D JSON Payload/main.js`: panel #map3dOutput - inferred from JS DOM lookup
...
Schema baseline: `tools/schemas/tools/3d-json-payload.schema.json`. Title: 3d-json-payload Payload. Required top-level fields: mapPayload. Allowed top-level fields: mapPayload. Additional top-level properties: rejected.
...
- publish to `tools.3d-json-payload` if applicable: yes, publish normalized output under `tools.3d-json-payload` when this tool is the producer.
...
Published output key: `tools.3d-json-payload`. The value must match the current contract baseline, contain only JSON-safe values, use file/path/name fields for assets, and never persist `imageDataUrl`. Games and samples should consume the published payload as data, not as workspace-managed tool internals.

## 11. Playwright Expectations
Open `tools/3D JSON Payload/index.html`; verify the page renders without console errors, expected controls are present, valid JSON/session data reaches the success state, and invalid JSON/session data stays in the tool-owned rejection path. No Playwright run is expected for this documentation-only PR.

## 12. Manual Test Expectations
Manually launch `tools/3D JSON Payload/index.html`, exercise import/load controls or hosted launch parameters, confirm edit/process controls do not delegate JSON internals to workspace, export/save the normalized output, and confirm invalid JSON blocks export/save.
...
P16: 3D JSON Payload. This priority is used by `docs/dev/roadmaps/MASTER_ROADMAP_TOOLS.md` and keeps the rebuild anchored on Palette / Palette Browser first, then dependent tool families.
