# svg-asset-studio-v2 Reengineering Design (svg-asset-studio-v2)
...
- Runtime entry point: `svg-asset-studio-v2/index.html`.
- Runtime implementation file: `tools/svg-asset-studio-v2/index.js`.
...
- Current fixture contract (no dedicated schema file in `tools/schemas/tools`): `version`, `toolId`, and `payloadJson` with `payloadJson.vectorAssetDocument`.
...
- Class `SvgAssetStudioV2` in `tools/svg-asset-studio-v2/index.js`.
- Class methods: `constructor()`, `goBack()`, `handleNavigationState()`, `readUrlState()`, `toolLabel()`, `renderNavigation()`, `buildToolUrl()`, `optionalUrlStateSummary()`, `handleSessionVersion()`, `buildRuntimeSnapshot()`, `registerSnapshotHook()`, `logStructuredError()`, `readSession()`, `loadContract()`, `renderSvg()`, `renderMissing()`, `renderError()`.
...
- Edit/process: `renderNavigation()`, `renderSvg()`, `renderMissing()`, `renderError()`
...
- Publish to `tools.svg-asset-studio-v2`: Supported as target ownership in design; concrete publish path varies by tool.
