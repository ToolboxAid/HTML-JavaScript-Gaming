# Tool Templates Reengineering Design

Task: PR_26124_021-tool-folder-design-reset
Tool ID: `templates`
Source folder: `tools/templates`

## 1. Tool Purpose
Provide starter project and vector-native template documentation/resources; not a launchable tool JSON editor.

## 2. Folder/Files Inspected
- `tools/templates/starter-project-template/README.md`
- `tools/templates/vector-native-arcade/docs/ROLLBACK_NOTES.md`
- `tools/templates/vector-native-arcade/docs/STARTER_GUIDE.md`
- `tools/templates/vector-native-arcade/README.md`
...
Template resources contain starter data, but this folder is not a launchable tool JSON editor and owns no `tools.*` output contract.

JSON handling signals found: download/export, tools.*, validate.
...
- publish to `tools.templates` if applicable: no standalone published output in this folder.
...
P31: Tool Templates. This priority is used by `docs/dev/roadmaps/MASTER_ROADMAP_TOOLS.md` and keeps the rebuild anchored on Palette / Palette Browser first, then dependent tool families.
