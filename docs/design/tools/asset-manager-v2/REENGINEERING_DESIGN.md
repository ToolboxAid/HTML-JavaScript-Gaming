# asset-manager-v2 Reengineering Design (asset-manager-v2)
...
- Runtime entry point: `asset-manager-v2/index.html`.
- Runtime implementation file: `tools/asset-manager-v2/index.js`.
...
- Current fixture contract (no dedicated schema file in `tools/schemas/tools`): `version`, `toolId`, and `payloadJson` with `payloadJson.assetCatalog`.
...
- Class `AssetBrowserV2` in `tools/asset-manager-v2/index.js`.
- Class methods: `constructor()`, `goBack()`, `openSvgAssetStudioV2()`, `handleNavigationState()`, `readUrlState()`, `toolLabel()`, `renderNavigation()`, `buildToolUrl()`, `optionalUrlStateSummary()`, `handleSessionVersion()`, `buildRuntimeSnapshot()`, `registerSnapshotHook()`, `cloneSessionValue()`, `setActionStatus()`, `clearSelectedAssetDetails()`, `renderSelectedAssetDetails()`, `normalizedAssetEntryFromForm()`, `clearAddAssetForm()`, `selectedAssetEntryFromCatalogEntries()`, `addAssetEntry()`, `removeAssetEntryById()`, `logStructuredError()`, `readSession()`, `loadContract()`, `persistValidSessionForWorkspace()`, `renderCatalog()`, `renderMissing()`, `renderError()`.
...
- Validate: `normalizedAssetEntryFromForm()`
- Edit/process: `renderNavigation()`, `renderSelectedAssetDetails()`, `clearAddAssetForm()`, `addAssetEntry()`, `removeAssetEntryById()`, `renderCatalog()`, `renderMissing()`, `renderError()`
...
- Publish to `tools.asset-manager-v2`: Supported as target ownership in design; concrete publish path varies by tool.
