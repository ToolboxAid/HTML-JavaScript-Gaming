...
function toProjectAssetUrl(value) {
...
function getOrderedImageSources(options = {}) {
  const sources = [];
  const transientSource = typeof options.transientSource === "string" && options.transientSource.trim()
    ? options.transientSource.trim()
    : "";
  const imageNameSource = toProjectAssetUrl(options.imageName || "");
  if (transientSource) {
    sources.push(transientSource);
  }
  if (imageNameSource) {
    sources.push(imageNameSource);
  }
  return sources;
}

function getPrimaryImageSource(options = {}) {
  const sources = getOrderedImageSources(options);
  return sources[0] || "";
}

function createPersistableTileMapDocument(documentModel) {
  return cloneDeep(documentModel);
}
...
function sanitizeTileset(rawTileset) {
  ...
  const sanitizeTileSource = (rawSource) => {
    ...
    const imageName = typeof rawSource.imageName === "string" ? rawSource.imageName.trim() : "";
    const hasImageName = imageName.length > 0;
    const inferredKind = rawSource.kind === "image" || hasImageName
      ? "image"
      : "atlas";
...
class TileMapEditorApp {
  ...
  clearTransientImageSources() {
    ...
    this.transientImageSourceByName.clear();
  }

  setTransientImageSource(imageName, objectUrl) {
    const key = typeof imageName === "string" ? imageName.trim() : "";
    if (!key || typeof objectUrl !== "string" || !objectUrl.trim()) {
      return;
    }
    const existing = this.transientImageSourceByName.get(key);
    if (typeof existing === "string" && existing.startsWith("blob:")) {
      URL.revokeObjectURL(existing);
    }
    this.transientImageSourceByName.set(key, objectUrl.trim());
  }

  getTransientImageSource(imageName) {
    const key = typeof imageName === "string" ? imageName.trim() : "";
    if (!key) {
      return "";
    }
    return this.transientImageSourceByName.get(key) || "";
  }

  getOrderedImageSourcesForName(imageName) {
    return getOrderedImageSources({
      imageName,
      transientSource: this.getTransientImageSource(imageName)
    });
  }
  ...
  handleSaveProject() {
    ...
    const persistableDocument = createPersistableTileMapDocument(this.documentModel);
    const output = addToolModeMetadata(persistableDocument, { toolId: "tile-map-editor" });
    output.project = {
  ...
  handlePackageProject() {
    ...
    const packageResult = buildProjectPackage({
      ...
      tileMapDocument: persistableDocument
    });
  ...
  async reloadTilesetImageFromDocument(options = {}) {
    ...
    const atlasImageSources = this.getOrderedImageSourcesForName(atlas.imageName);
    if (atlasImageSources.length === 0) {
      this.tilesetImage = null;
      this.renderTileset();
    ...
    let lastError = null;
    for (let index = 0; index < atlasImageSources.length; index += 1) {
      try {
        loadedImage = await loadImageElement(atlasImageSources[index]);
        break;
      } catch (error) {
        lastError = error;
      }
    ...
    if (loadedImage) {
      ...
      atlas.imageWidth = loadedImage.naturalWidth || atlas.imageWidth;
      atlas.imageHeight = loadedImage.naturalHeight || atlas.imageHeight;
      this.documentModel.tilesetAtlas = atlas;
      this.renderTileset();
      this.renderTilesetMeta();
      this.renderCanvas();
    ...
    this.renderCanvas();
    if (!quiet) {
      this.updateStatus(`Tileset PNG failed to load: ${lastError instanceof Error ? lastError.message : "unknown error"}`);
    }
  }
  ...
  async preloadIndividualTileImages(options = {}) {
    ...
    for (let index = 0; index < this.documentModel.tileset.length; index += 1) {
      ...
      const resolvedSource = getPrimaryImageSource({
        imageName: source.imageName,
        transientSource: this.getTransientImageSource(source.imageName)
      });
      if (!resolvedSource) {
        continue;
      }
      if (loadedUrls.has(resolvedSource)) {
        continue;
      }

      loadedUrls.add(resolvedSource);
      if (this.tilesetImageCache.has(resolvedSource)) {
        continue;
      }

      try {
        const image = await loadImageElement(resolvedSource);
        this.tilesetImageCache.set(resolvedSource, image);
      } catch (error) {
        if (!quiet) {
  ...
  async handleLoadTilesetPng(event) {
    ...
    try {
      ...
      this.documentModel.tilesetAtlas = atlas;
      this.setTransientImageSource(atlas.imageName, objectUrl);
  ...
  async handleLoadTilePngAssets(event) {
    ...
    for (let index = 0; index < files.length; index += 1) {
      ...
      try {
        ...
        const imageName = file.name || `${safeName}.png`;
        this.setTransientImageSource(imageName, objectUrl);
        ...
        const cacheKey = getPrimaryImageSource({
          imageName,
          transientSource: this.getTransientImageSource(imageName)
        });
        if (cacheKey) {
          this.tilesetImageCache.set(cacheKey, image);
        }
        importedCount += 1;
  ...
  async generateTilesetFromLoadedPng() {
    ...
    if (!image) {
      ...
      if (loadedImage) {
        ...
        const failureMessage = `Tileset image decode failed: ${lastError instanceof Error ? lastError.message : "unknown error"}`;
        this.updateStatus(failureMessage);
        if (typeof globalThis.alert === "function") {
          globalThis.alert(failureMessage);
        }
  ...
  getTileSourceDrawInfo(tile) {
    ...
    if (sourceKind === "image") {
      ...
      if (!imageSource || !this.tilesetImageCache.has(imageSource)) {
        return null;
      }
      const image = this.tilesetImageCache.get(imageSource);
  ...
  renderTilesetMeta() {
    ...
    const importedPngCount = this.documentModel.tileset.filter((tile) => tile.id > 0 && tile?.source?.kind === "image").length;
    const atlasSource = getPrimaryImageSource({
      imageName: atlas.imageName,
      transientSource: this.getTransientImageSource(atlas.imageName)
    });
    if (!atlasSource) {
