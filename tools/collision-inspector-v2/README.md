...
Collision Inspector V2 loads game manifests and inspects collision behavior through the shared engine Object Vector collision path. It uses Object Vector Studio V2 `objects[].shapes[]` geometry only, and it does not create fallback vector maps or hardcoded game geometry.

## Primary Flow

1. Open `tools/collision-inspector-v2/index.html`.
2. Load a game manifest from the header file picker, pass a `manifestPath` URL parameter, or launch from Workspace Manager V2.
