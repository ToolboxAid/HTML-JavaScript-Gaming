...
export default class TrustSessionValidationScene extends Scene {
  ...
  render(renderer) { drawFrame(renderer, theme, ['Engine Sample170', 'Session trust validation catches invalid or stale session state early.', this.status]); drawPanel(renderer, 120, 220, 560, 220, 'Session Trust', [`Passed: ${this.result?.passed ?? 'n/a'}`, `Detail: ${this.result?.detail || 'none'}`, `Age: ${this.result?.ageMs ?? 0}ms`]); }
}
