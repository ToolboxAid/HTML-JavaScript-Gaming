...
const FALLBACK_PREVIEW_DATA_URL =
  "data:image/svg+xml;utf8," +
  encodeURIComponent(
    "<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 640 360'><rect width='100%' height='100%' fill='#101829'/><rect x='18' y='18' width='604' height='324' rx='10' fill='none' stroke='#3a4f72'/><text x='50%' y='52%' text-anchor='middle' dominant-baseline='middle' fill='#8fa8cb' font-family='monospace' font-size='24'>No Preview</text></svg>"
  );
...
export function resolveThumbnailSource(sample) {
  if (!sample || typeof sample !== "object") {
    return FALLBACK_PREVIEW_DATA_URL;
  }
  return sample.thumbnail || sample.preview || FALLBACK_PREVIEW_DATA_URL;
}

export function resolvePreviewSource(sample) {
  if (!sample || typeof sample !== "object") {
    return FALLBACK_PREVIEW_DATA_URL;
  }
  return sample.preview || sample.thumbnail || FALLBACK_PREVIEW_DATA_URL;
}
...
export function buildDetailModel(samples, currentId) {
  ...
  const related = list
    ...
    .sort(
      (a, b) =>
        numericDistanceFromCurrent(current.id, a.id) - numericDistanceFromCurrent(current.id, b.id) ||
        sortById(a, b)
    )
    .slice(0, 3);
...
function ensureStyles() {
  ...
  style.textContent = [
    ...
    ".sample-detail-enhancement figure.is-fallback img { opacity: 0.88; }"
  ].join("\n");
...
function buildEnhancementElement(model) {
  ...
  image.src = resolvePreviewSource(model.current);
  image.alt = "Preview for sample " + model.current.id;
  if (!model.current.preview && !model.current.thumbnail) {
    figure.className = "is-fallback";
  }
