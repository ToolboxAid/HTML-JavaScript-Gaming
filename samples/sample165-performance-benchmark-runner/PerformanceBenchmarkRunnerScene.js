...
import Scene from '../../engine/scenes/Scene.js'; import { drawFrame, drawPanel } from '../../engine/debug/index.js'; import { Theme, ThemeTokens } from '../../engine/theme/index.js'; import { BenchmarkRunner } from '../../engine/automation/index.js';
...
export default class PerformanceBenchmarkRunnerScene extends Scene {
  ...
  render(renderer) { drawFrame(renderer, theme, ['Engine Sample165', 'Benchmarks run through one reusable execution path for comparison.', this.status]); drawPanel(renderer, 120, 220, 520, 220, 'Benchmarks', this.results.length ? this.results.map((result) => `${result.id}: ${result.durationMs.toFixed(3)}ms`) : ['No benchmarks run yet.']); }
}
