...
export default class BuildAssetManifestSystemScene extends Scene {
  ...
  render(renderer) { drawFrame(renderer, theme, ['Engine Sample175', 'Build manifests describe included content in a reusable structure.', this.status]); drawPanel(renderer, 120, 220, 560, 220, 'Manifest', [`Build: ${this.manifest?.buildId || 'n/a'}`, `Assets: ${this.manifest?.assetCount ?? 0}`, `Valid: ${this.validation?.passed ?? 'n/a'}`]); }
}
